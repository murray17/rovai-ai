...
- v0.05 的五个实施检查点已经完成：AgentRun 输入可冻结重现，Native Session 只接收未读公共增量，压缩仅在预算超限时触发；`team.post_message` 已支持 Codex、OpenCode 与 Copilot 的可恢复 A2A 执行链。
...
- 本机验收已覆盖四种 Runtime 的真实最小执行、Codex/ACP 审批、双 Agent 隔离、Core 重启恢复、Codex/OpenCode/Copilot A→B→A 显式回信，以及 AGY 同 Session 续接和 AGY → Codex 跨 Adapter 换绑；AGY 暂不支持 Team Tool。

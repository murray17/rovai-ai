...
Rovai-ai 是一个本地优先的多 Agent 研发工作空间，通过桌面应用组织长期队员、Camp 协作、任务执行、权限审批、审计与恢复，并驱动用户本机已有的 Coding Agent CLI。
...
- v0.03 的五个实施检查点已经完成：队员管理、共享 `AdapterInstallation`、动态模型目录、Adapter 原生权限与 Native Session 惰性交接均已落地。
...
- v0.06 的五个实施检查点已经完成：用户与 Agent 可管理长期 Task，分配不会隐式唤醒队员；三个 Task Team Tool、授权读取、乐观并发、`[TASK_CONTEXT]`、真实双 Agent 交接和安全恢复均已落地。
- v0.07 的五个实施检查点已经完成：Hearth & Camp 双主题、系统/白昼/夜间偏好、稳定队员身份色、证据工作区与完整 Day/Night App 验收均已落地。
...
- v0.09 的五个实施检查点已经完成：文件型 MCP Library、六种本机配置 Importer、按队员分配、AgentRun 冻结投影、四种 Runtime 原生注入和结构化设置页均已落地；Rovai-ai 不预装第三方 MCP，也不修改用户的 Runtime 配置。
...
- v0.15 已完成队员生命周期、保留式永久移除、Camp Default Lead
...
- v0.24 至 v0.30 已冻结为历史版本；Arctic Dawn V3、持久 Composer Draft、队员级 Runtime
...
- v0.35 至 v0.42 已冻结为历史版本；Native Session Identity Bootstrap、诊断组合、标准 MCP、
  Task 实时卡片、Runtime 隔离、Camp 历史工具、统一运行活动和 built-in CLI transport 已落地。
- v0.43 是唯一当前版本：外部 MCP 使用 Runtime-native additive projection，Codex 回到原生 Home；
  当前领域契约同时统一为应用全局 `Member / 队员`、公开 `agentId`、`Task`，Camp 只保留存在或
  永久删除两种结果，Project 只作为读取时派生分组。以
  [v0.43 实施计划](docs/versions/v0.43/implementation-plan.md)为准。
...
- [v0.15 队员生命周期与 Camp 执行准入](docs/versions/v0.15/README.md)
...
- [v0.26 队员级 Runtime 参数与后台检查](docs/versions/v0.26/README.md)
...
- [v0.43 Runtime-Native Additive MCP 与领域契约收敛（当前）](docs/versions/v0.43/README.md)

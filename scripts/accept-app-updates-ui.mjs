...
async function openAboutUpdates(cdp) {
  ...
  await waitForExpression(cdp,
    `document.querySelector('.about-identity p > span:first-child')?.textContent === '版本 v0.2.3'`)
}

async function assertAboutUpdates(cdp, context) {
  const updaterSnapshot = await evaluate(cdp, 'window.rovai.appUpdates.get()', true)
  assert(updaterSnapshot?.currentVersion === '0.2.3'
  ...
  assert(state.product === 'Rovai AI' && state.version === '版本 v0.2.3',

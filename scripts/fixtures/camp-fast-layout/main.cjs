...
app.whenReady().then(async () => {
  ...
  try {
    ...
    assert.equal(state.pressed, 'mixed', 'A prior forced Fast observation is not an inherited default')
    ...
    await click('.camp-member-action-button')
    await run(`document.querySelectorAll('[role="menuitem"]').forEach(node => { if(node.textContent.includes('检测响应模式')) node.setAttribute('data-fixture-check','true') })`)
    await click('[data-fixture-check]')
    ...
    await run('window.fastTest.restore()')
    await snapshot()
    await run('window.fastTest.holdNext()')

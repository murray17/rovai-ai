...
app.whenReady().then(async () => {
  ...
  try {
    ...
    await settle()
    await click('.host-web-actions .primary-button')
    await run('window.settingsTest.releaseHostStatus()'); await settle()
    assert.equal(await run("document.querySelector('.host-web-settings input[readonly]').type"), 'password')
    assert.equal(await run("document.querySelector('.host-web-settings input[readonly]').value"), 'fixture-token-1')
    assert.ok(await run("document.querySelector('.host-web-settings [role=status]').textContent.includes('已开启')"), 'late status must not undo start')
    await capture('host-web-enabled-day')
    await click('.host-web-actions button:first-child')
    assert.equal(await run("document.querySelector('.host-web-settings input[readonly]').value"), 'fixture-token-2')
    await click('.host-web-actions button:last-child')
    assert.equal(await run("document.querySelectorAll('.host-web-settings input[readonly]').length"), 0)
    await run('window.settingsTest.state.loseHostStartReply = true')
    await click('.host-web-actions .primary-button')
    assert.ok(await run("document.querySelector('.host-web-settings [role=status]').textContent.includes('已开启')"))
    ...
    assert.equal(await run("document.querySelectorAll('.host-web-settings input[readonly]').length"), 0, 'remount must not recover a management token from persistence')
    await click('.host-web-actions button:last-child')

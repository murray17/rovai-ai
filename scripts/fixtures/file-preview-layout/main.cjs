...
app.whenReady().then(async () => {
  ...
  await check('drag-to-close keeps the preferred ratio, file tab, reading position and draft', async () => {
    ...
    assert.equal(closed.tabCount, 0, 'No separate preview entry remains in the header')
  ...
  await check('compact find owns the tool area and approval/recovery stay above the same Composer', async () => {
    ...
    assert.equal(docked.approvalTextWraps, true)
    ...
    closeTo(docked.approval.width, 396, 'Approval follows the narrow conversation')

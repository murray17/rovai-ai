...
Object.assign(window, { dingtalkLoginTest: {
  async run() {
    ...
    check(image?.complete && image.naturalWidth === 280, 'The Main QR PNG must be displayed')
    ...
    await stage('awaiting_refresh')
    check(!document.querySelector('[role="alert"]'), 'A local timeout must not become a page error')
    check(!document.querySelector('[role="dialog"]')?.textContent?.match(/超时|已过期/u), 'Refresh does not claim server expiry')
    check(button('刷新二维码').matches('.channel-qr-frame'), 'The original QR region remains the retry entry point')
    button('刷新二维码').click()
    await settle()
    check((await window.loginFixture.facts()).refreshes === 2, 'A timed-out attempt refreshes from the same dialog')
    cases.push('local timeout preserves the dialog and QR refresh action without an error')

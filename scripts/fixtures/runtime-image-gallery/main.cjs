...
app.whenReady().then(async () => {
  ...
  try {
    ...
    assert.equal(await run('window.imageGalleryTest.state().dialog'), false, 'Automatic thumbnail loading must not open a lightbox')
    assert.equal(await run('window.imageGalleryTest.attachmentCalls.length'), 2, 'Visible unknown attachments load without clicking')
    ...
    assert.equal(current.columns, 3)
    assert.equal(current.overflow, false)
    assert.deepEqual(current.fit, ['contain', 'contain'])
    assert.deepEqual(current.userFit, ['cover'])
    assert.ok(current.userFrames.every(frame => Math.abs(frame.width - 70) < 1 && Math.abs(frame.height - 70) < 1),
      `User thumbnails must keep the 72px square tile including its border: ${JSON.stringify(current.userFrames)}`)
    ...
    // Unknown attachments remain lazy: offscreen mounting alone must not read the file.
    const readsBeforeScroll = await run('window.imageGalleryTest.attachmentCalls.length')
    await run("window.imageGalleryTest.showScenario('attachment', 'cache-offscreen', 2400)")
    await run('new Promise(resolve => setTimeout(resolve, 100))')
    assert.equal(await run('window.imageGalleryTest.attachmentCalls.length'), readsBeforeScroll)
    await run('document.querySelector(".image-tile").scrollIntoView()')
    await waitFor('window.imageGalleryTest.scenarioState().decoded === 1')
    assert.equal(await run('window.imageGalleryTest.attachmentCalls.length'), readsBeforeScroll + 1)
    ...
    assert.equal((await run('window.imageGalleryTest.scenarioState()')).attachmentReads, attachmentCold.attachmentReads + 1)

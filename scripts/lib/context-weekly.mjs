...
export function weeklyPoint(week, attempt, report) {
  ...
  return { week, attempt, locator: `${week}/${attempt}/report.json`, status: report.status,
    ...
    sampleSize: slots.length, completed: completed.length, hardPass, hardFail, unknown: slots.length - hardPass - hardFail,
...
export async function updateWeeklyTrend(root) {
  ...
  const data = { schemaVersion: 1, selection: 'first_attempt_per_week', points, selected, limits: ['Hard pass is not semantic acceptance or user task success.', 'Missing, partial or changed environments are not connected.', 'Each attempt keeps its denominator; reruns never replace the first attempt.'] }
  for (const [name, content] of [['trend-data.json', `${JSON.stringify(data, null, 2)}\n`], ['trends.svg', renderWeeklyTrend(selected)], ['README.md', `# 每周真实任务回归趋势\n\n曲线使用每周第一次尝试，重跑单列保留；未运行、缺证据与环境变化不连线。每周以 UTC 周一标识，实际触发时区由 Automation 配置。硬性通过不代表 Judge 通过或用户任务成功。\n\n![每周曲线](trends.svg)\n\n[完整趋势与版本](trend-data.json)\n\n| 周 | 尝试 | 结论 | 硬性通过/计划样本 | 证据缺口 |\n|---|---|---|---|---|\n${points.map(p => `| ${p.week} | [${p.attempt}](${p.locator}) | ${p.status} | ${p.hardPassRate.numerator ?? '?'}/${p.sampleSize ?? '?'} | ${p.evidenceGaps ?? '?'} |`).join('\n')}\n`]]) {
...
export function renderWeeklyTrend(points) {
  ...
  points.forEach((point, index) => {
    ...
    const x = 65 + 860 * index / Math.max(1, points.length - 1), y = 210 - value * 150

...
function contentAllowed(segment, configuration) {
  ...
  if (![digest(RECEIPT_REDACTION_POLICY), digest(WITNESS_REDACTION_POLICY), digest(DELIVERY_REDACTION_POLICY)].includes(configuration.payload.redactionPolicyDigest)) return false
...
export function buildSemanticJudgeConfiguration({
  ...
  if (evaluationContextPolicy !== null && !['bounded-evaluation-context-v1', 'bounded-evaluation-context-v2', 'bounded-evaluation-context-v3'].includes(evaluationContextPolicy)) throw new Error('Unsupported evaluation context policy')
  ...
  const payload = {
    ...
    redactionPolicyDigest: digest(evaluationContextPolicy === 'bounded-evaluation-context-v3' ? DELIVERY_REDACTION_POLICY : evaluationContextPolicy === 'bounded-evaluation-context-v2' ? WITNESS_REDACTION_POLICY : evaluationContextPolicy ? RECEIPT_REDACTION_POLICY : CURRENT_REDACTION_POLICY),
...
export function validateSemanticJudgeConfiguration(artifact) {
  ...
  if (![digest(LEGACY_REDACTION_POLICY), digest(CURRENT_REDACTION_POLICY), digest(RECEIPT_REDACTION_POLICY), digest(WITNESS_REDACTION_POLICY), digest(DELIVERY_REDACTION_POLICY)]
...
export function buildJudgeEvidencePack({
  ...
  if ([digest(CURRENT_REDACTION_POLICY), digest(RECEIPT_REDACTION_POLICY), digest(WITNESS_REDACTION_POLICY), digest(DELIVERY_REDACTION_POLICY)].includes(configuration.payload.redactionPolicyDigest)) {

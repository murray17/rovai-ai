...
export function validateScoring(scoring, cases) {
  if (scoring?.schemaVersion !== 1 || !['2.0.0', '2.1.0', '2.2.0', '2.3.0', '2.4.0'].includes(scoring.version) || scoring.id !== 'generic-task-quality'
  ...
  for (const spec of cases) {
    ...
    if (['2.1.0', '2.2.0', '2.3.0', '2.4.0'].includes(scoring.version) && (config?.judgeProfile !== (scoring.version === '2.4.0' ? 'generic-task-v6' : scoring.version === '2.3.0' ? 'generic-task-v5' : scoring.version === '2.2.0' ? 'generic-task-v4' : 'generic-task-v3') || !Array.isArray(config.evidenceFiles)
    ...
    if (['2.3.0', '2.4.0'].includes(scoring.version)) for (const item of [...config.quality, ...config.collaboration]) validateMetricContract(item)

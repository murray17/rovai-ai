...
import { HISTORY_OUTCOME_RUBRIC, SOURCE_OUTCOME_RUBRIC, EXECUTION_OUTCOME_RUBRIC, DELIVERY_OUTCOME_RUBRIC, WITNESS_OUTCOME_RUBRIC, CLAIM_OUTCOME_RUBRIC, CLAIM_PROCESS_RUBRIC, OBSERVABLE_OUTCOME_RUBRIC, OBSERVABLE_PROCESS_RUBRIC, TASK_OUTCOME_RUBRIC, EVIDENCE_OUTCOME_RUBRIC, EVIDENCE_PROCESS_RUBRIC, RECEIPT_OUTCOME_RUBRIC, RECEIPT_PROCESS_RUBRIC } from './context-judge-profile.mjs'
...
export async function freezePlan(config, output) {
  ...
  if (judge) {
    ...
    if (['2.7.0', '2.8.0', '2.9.0'].includes(scoring.version) && !adapter.claimAuditProfiles?.includes('claim-audit-v4')) throw new Error('v9 scoring requires claim-audit-v4 support')
  ...
  const plan = { schemaVersion: 1, createdAt: new Date().toISOString(), mode: config.mode, change: { ...config.change, document: resolve(config.change.document), documentDigest: digestJson(document) },
    ...
    rubricDigest: digestJson({ process: ['2.4.0', '2.5.0', '2.6.0', '2.7.0', '2.8.0', '2.9.0'].includes(scoring.version) ? CLAIM_PROCESS_RUBRIC : scoring.version === '2.3.0' ? OBSERVABLE_PROCESS_RUBRIC : scoring.version === '2.2.0' ? RECEIPT_PROCESS_RUBRIC : scoring.version === '2.1.0' ? EVIDENCE_PROCESS_RUBRIC : PROCESS_JUDGE_RUBRIC, outcome: scoring.version === '2.9.0' ? HISTORY_OUTCOME_RUBRIC : scoring.version === '2.8.0' ? SOURCE_OUTCOME_RUBRIC : scoring.version === '2.7.0' ? EXECUTION_OUTCOME_RUBRIC : scoring.version === '2.6.0' ? DELIVERY_OUTCOME_RUBRIC : scoring.version === '2.5.0' ? WITNESS_OUTCOME_RUBRIC : scoring.version === '2.4.0' ? CLAIM_OUTCOME_RUBRIC : scoring.version === '2.3.0' ? OBSERVABLE_OUTCOME_RUBRIC : scoring.version === '2.2.0' ? RECEIPT_OUTCOME_RUBRIC : scoring.version === '2.1.0' ? EVIDENCE_OUTCOME_RUBRIC : TASK_OUTCOME_RUBRIC, scoring }), evaluatorDigest: await evaluatorDigest(),

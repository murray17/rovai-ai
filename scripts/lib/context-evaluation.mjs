...
export async function freezePlan(config, output) {
  ...
  if (judge) {
    ...
    if (scoring.version === '2.7.0' && !adapter.claimAuditProfiles?.includes('claim-audit-v4')) throw new Error('v9 scoring requires claim-audit-v4 support')
  ...
  const plan = { schemaVersion: 1, createdAt: new Date().toISOString(), mode: config.mode, change: { ...config.change, document: resolve(config.change.document), documentDigest: digestJson(document) },
    ...
    rubricDigest: digestJson({ process: ['2.4.0', '2.5.0', '2.6.0', '2.7.0'].includes(scoring.version) ? CLAIM_PROCESS_RUBRIC : scoring.version === '2.3.0' ? OBSERVABLE_PROCESS_RUBRIC : scoring.version === '2.2.0' ? RECEIPT_PROCESS_RUBRIC : scoring.version === '2.1.0' ? EVIDENCE_PROCESS_RUBRIC : PROCESS_JUDGE_RUBRIC, outcome: scoring.version === '2.7.0' ? EXECUTION_OUTCOME_RUBRIC : scoring.version === '2.6.0' ? DELIVERY_OUTCOME_RUBRIC : scoring.version === '2.5.0' ? WITNESS_OUTCOME_RUBRIC : scoring.version === '2.4.0' ? CLAIM_OUTCOME_RUBRIC : scoring.version === '2.3.0' ? OBSERVABLE_OUTCOME_RUBRIC : scoring.version === '2.2.0' ? RECEIPT_OUTCOME_RUBRIC : scoring.version === '2.1.0' ? EVIDENCE_OUTCOME_RUBRIC : TASK_OUTCOME_RUBRIC, scoring }), evaluatorDigest: await evaluatorDigest(),

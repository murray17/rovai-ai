...
test('remote connection design preserves production settings geometry, keyboard validation and reversible decisions', { timeout: 120_000 }, async t => {
  ...
  try {
    ...
    await input('#remote-origin', 'http://0.0.0.0:4321')
    await browser.click(button('开启浏览器访问'))
    await browser.wait(`document.activeElement.id === 'remote-origin'`)
    await input('#remote-origin', 'http://192.168.1.12:4321')
    await browser.click(button('开启浏览器访问'))
    await browser.wait(`document.activeElement.id === 'remote-risk'`)
    await browser.click(`document.querySelector('#remote-risk')`)
    await browser.click(button('开启浏览器访问'))
    ...
    await browser.click(button('更换令牌'))
    await dialogReady()
    ...
    await browser.click(button('更换令牌')); await dialogReady(); await browser.click(button('更换令牌并退出会话'))
    await browser.wait(`document.querySelector('[role="dialog"]')===null`)
    ...
    assert.equal(await browser.evaluate(`document.querySelector('#remote-token')===null`), true, 'leaving the page clears one-time token display')
    assert.match(await browser.evaluate('document.body.innerText'), /0 个已登录会话/)
    ...
    await browser.click(`document.querySelector('[aria-label="开启浏览器访问"]')`)
    await dialogReady()
    await browser.click(button('关闭浏览器访问'))
    await browser.wait(`document.querySelector('#remote-port')!==null`)
    assert.equal(await browser.evaluate(`document.querySelector('#remote-port').value`), '4321')
    evidence.checks.push('field-validation-focus-and-edit-retention', 'explicit-LAN-consent', 'submitting-disables-repeat', 'token-masked-and-cleared-on-navigation', 'rotate-and-stop-cancel-and-confirm')

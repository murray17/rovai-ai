...
test('Product Contract Fingerprint reads code/build authority and marks unavailable data explicitly', async () => {
  ...
  assert.equal(fingerprint.dataContractVersion.value, 'v0.67')
  assert.equal(fingerprint.dataContractSchemaVersion.value, 33)
  ...
  assert.equal(fingerprint.builtInTransportVersion.value, 7)

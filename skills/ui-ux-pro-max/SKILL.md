---
name: ui-ux-pro-max
description: "Use when designing, implementing, reviewing, or improving web, mobile, or desktop interfaces. Continue using it for visual, interaction, accessibility, responsive, typography, color, and stack-specific UI refinements. Do not use for backend-only logic, APIs, databases, infrastructure, or non-visual scripts."
---
...
For the full rule list per category (all 119 UX guidelines with rationale), read [the quick reference](references/quick-reference.md). For app-specific polish rules (icons, touch feedback, dark mode contrast, safe areas) and the canonical pre-delivery checklist, read [the professional rules](references/pro-rules.md).

---

## Running the search tool

The search script lives inside this Skill's own directory. In every command below, replace `<skill-root>` with the absolute directory containing the `SKILL.md` you loaded from the Runtime. Use that same copy for scripts, references, and data; do not assume a project working directory, provider-specific plugin variable, or user-global installation.

Use Python's `-B` option to avoid writing bytecode into the immutable Skill Revision. Keep generated design-system output in the target project via `--output-dir`, never inside this Skill bundle:

```bash
python3 -B "<skill-root>/scripts/search.py" "<query>" --domain <domain>
```

Requires Python 3.x and its standard library only; no pip packages or network access are needed for searches. If `python3` is unavailable, use `python -B` when it is Python 3, or `py -3 -B` on Windows. If none is available, report the missing prerequisite without installing software automatically.
...
```bash
python3 -B "<skill-root>/scripts/search.py" "<product_type> <industry> <keywords>" --design-system [-p "Project Name"]
```
...
**Example:**
```bash
python3 -B "<skill-root>/scripts/search.py" "beauty spa wellness service" --design-system -p "Serenity Spa"
```
...
```bash
python3 -B "<skill-root>/scripts/search.py" "<query>" --design-system --persist -p "Project Name" --output-dir "<project-root>"
```
...
```bash
python3 -B "<skill-root>/scripts/search.py" "<query>" --design-system --variance <1-10> --motion <1-10> --density <1-10>
```
...
**Example:**
```bash
python3 -B "<skill-root>/scripts/search.py" "internal analytics dashboard" --design-system --variance 8 --motion 7 --density 8 -p "Ops Console"
```

### Step 3: Supplement with Detailed Searches (as needed)

```bash
python3 -B "<skill-root>/scripts/search.py" "<keyword>" --domain <domain> [-n <max_results>]
```
...
```bash
python3 -B "<skill-root>/scripts/search.py" "<keyword>" --stack <stack>
```
...
python3 -B "<skill-root>/scripts/search.py" "AI search tool modern minimal" --design-system -p "AI Search"

# Step 3: supplement
python3 -B "<skill-root>/scripts/search.py" "keyboard focus modal" --domain ux

# Step 4: stack guidelines
python3 -B "<skill-root>/scripts/search.py" "suspense streaming bundle" --stack nextjs
```

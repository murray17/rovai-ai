...
综合时，才使用 Gather。V1 对所有成员使用同一 `body`；`to` 应使用 canonical Agent ID，重复目标由 Core
冻结去重。
...
显式回传只是结果证据，不代表成员工作已经结束。成员 Run 终态才关闭对应 Item；没有显式回传时，
Rovai 使用有界最终总结兜底。所有 Item 终态后，Rovai 把一份完整冻结结果作为 mandatory Current Input，

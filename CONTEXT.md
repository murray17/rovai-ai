...
A model-visible navigation hint emitted only when one or more whole eligible public messages do not enter Shared Conversation. It reports the exact omitted count and the omitted set's minimum/maximum sequence envelope without claiming that the sequence is continuous or directly executable; exact omitted message IDs and reasons remain Context Projection Evidence. Body truncation on an included message is a separate fact and provides a canonical `camp.read` item continuation rather than a range locator.

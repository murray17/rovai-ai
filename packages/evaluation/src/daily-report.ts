...
export const DAILY_SERIES = [
  ...
  ['A2A 未结束交接','a2a.openCountAsOf','count'],
...
export function reportMetrics(input: unknown): Record<string,unknown> {
  ...
  a2a.openCountAsOf=valid(coverage.numerator)&&valid(coverage.denominator)&&coverage.denominator>=coverage.numerator?coverage.denominator-coverage.numerator:null
  metrics.a2a=a2a
  metrics.reportDefinitionVersion=2
  return metrics
}
...
export function renderDailyHtml(input:unknown,history:unknown,factInput:unknown,analysis:unknown=null):string {
  ...
  const tools=table(['工具来源','成功','失败','拒绝','取消','未执行','未知','仍在进行','错误分类'],['core','runtime'].map(source=>{
    const t=obj(at(metrics,`tools.bySource.${source}`)),terminal=t.terminalOutcomesInWindow
    return[source,...['succeeded','failed','denied','cancelled','not_executed','unknown'].map(key=>number(terminal&&typeof terminal==='object'?obj(terminal)[key]===undefined?0:obj(terminal)[key]:null)),number(t.inFlightAsOf),escapeHtml(JSON.stringify(t.failureCodes??null))]
  }))

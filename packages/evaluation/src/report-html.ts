...
const conclusionLabels: Record<string,string> = {
  passed:'通过', failed:'未通过', incomplete:'存在证据缺口', complete:'评价完整',
...
const issueLabels: Record<string,string> = {
  ...
  candidate_contract_failure:'合同测试失败', contract_checks_not_passed:'合同测试未完成通过'
}
function gateConclusions(report: Record<string,unknown>): string {
  ...
  return `<section id="conclusions"><h2>本轮结论</h2>${table(['验收结果','新旧比较','评价完整性'],[[escapeHtml(conclusionLabels[String(c.acceptance)]??c.acceptance),escapeHtml(conclusionLabels[String(c.regression)]??c.regression),escapeHtml(conclusionLabels[String(c.evaluation)]??c.evaluation)]])}<p>验收失败涉及 ${number(c.failedTrials)} 次 Trial；证据缺口涉及 ${number(c.evidenceGapTrials)} 次 Trial。两类可重叠，记录数不等于 Case 数。${c.regression==='not_compared'?'本轮只有当前版本，不能判断是否由改动造成退化。':''}</p></section>`
}
function issueSummary(report: Record<string,unknown>): string {
  const problems: Record<string,unknown>[]=[...rows(report.regressions).map(row=>({...row,issueType:row.newRegression===true?'已确认新退化':'验收未满足'})),...rows(report.evidenceGaps).map(row=>({...row,issueType:'证据缺口'}))]
  if(!problems.length)return '<p>未记录验收失败或证据缺口。</p>'
  return table(['Case / 重复','问题类型','原因与检查项'],problems.map(row=>[
    ...
    escapeHtml(row.issueType),`${escapeHtml(issueLabels[String(row.code)]??row.code)}<br><small>${escapeHtml(row.checklistItem??(Array.isArray(row.items)?row.items.join(', '):''))}</small>`
  ]))
}
export function renderGateHtml(input: unknown): string {
  ...
  const caseOverview=table(['Case / 任务','版本 / 重复','硬性验收','语义评价','质量 / 覆盖','执行耗时'],slots.map(slot=>{
    ...
    return [`<a href="#case-${escapeHtml(slot.caseId)}">${escapeHtml(slot.caseId)}</a><br><small>${escapeHtml(slot.caseTitle??'')}</small>`,`${escapeHtml(slot.arm)} / ${number(slot.repeat)}`,accepted?'通过':hardFailed?'未通过':'证据不足',state(slot.judgeStatus),`${number(at(trial,'quality.total'))} / 100<br>${ratioText(at(trial,'quality.coverage'))}`,`${number(typeof at(slot,'resources.dispatchToTerminal.valueMilliseconds')==='number'?Number(at(slot,'resources.dispatchToTerminal.valueMilliseconds'))/1000:null)} 秒`]
  }))

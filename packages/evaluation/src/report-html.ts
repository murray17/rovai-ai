...
export function renderGateHtml(input: unknown): string {
  ...
  const details=caseIds.map(id=>{
    const trialDetails=Object.entries(arms).flatMap(([arm,value])=>rows(obj(value).trials).filter(trial=>trial.caseId===id).map(trial=>{
      ...
      return `<h3>${arm==='baseline'?'基线':'候选 / 当前'} · repeat ${number(trial.repeat)} · ${state(trial.executionState)}</h3><p>质量 ${number(at(trial,'quality.total'))} / 100；覆盖 ${ratioText(at(trial,'quality.coverage'))}</p>${detailTable(['质量检查 / 依据','判定','来源 / 理由'],rows(at(trial,'quality.items')).map(item=>[`${escapeHtml(item.id)}<br><small>${escapeHtml(item.criterion)}</small>`,state(item.verdict),`${escapeHtml(item.source)}<br>${escapeHtml(at(item,'raw.reason')??item.reasonCode)}`]))}<h3>协作五个细项</h3>${detailTable(['细项','判定','理由与证据'],rows(trial.collaboration).map(item=>[escapeHtml(item.checklistItem),state(item.verdict),`${escapeHtml(at(item,'raw.reason')??item.reasonCode)}<details><summary>原始 Judge 判定与证据引用</summary>${json(item.raw)}</details>`]))}<details><summary>规则、资源与执行限制</summary>${json(raw)}</details><p>${links}</p>`
    }))
    return `<details data-case="${escapeHtml(id)}"><summary>${escapeHtml(id)}</summary>${trialDetails.length?trialDetails.join(''):json(slots.filter(s=>s.caseId===id))}</details>`
  }).join('')
  return htmlPage(report.kind==='weekly_regression'?'每周真实任务回归':'上下文改动 Gate',`${report.fixture === true ? '<p class="notice">合成测试夹具 · 仅验证报告呈现与交互，不是真实评测结果。</p>' : ''}<header><h1>${report.kind==='weekly_regression'?'每周真实任务回归':'上下文改动 Gate'}</h1><p>固定任务的实际产物与协作证据，用于检查改动后的质量和退化。</p><nav><a href="#quality">任务质量</a><a href="#collaboration">协作诊断</a><a href="#cases">Case 与证据</a><a href="#limits">限制与版本</a>${evidenceLink('report.json','原始报告')}${evidenceLink('plan.json','冻结计划')}</nav></header><div class="summary"><div>Gate 结论<strong>${state(report.status)}</strong></div><div>通用质量<strong>${number(at(candidate,'quality.total'))} / 100</strong></div><div>硬性通过<strong>${hard} / ${slots.filter(s=>s.arm==='candidate').length}</strong></div><div>质量评价覆盖<strong>${ratioText(at(candidate,'quality.coverage'))}</strong></div></div>${legacy?'<p class="notice">历史报告未使用当前评分标准。保留旧结果，不能直接换算新分数或连接质量趋势。</p>':''}<p>关键协作：不满足 ${number(at(candidate,'collaboration.criticalFailureTrials'))} 次 Trial；部分满足 ${number(at(candidate,'collaboration.criticalPartialTrials'))}；未知 ${number(at(candidate,'collaboration.criticalUnknownTrials'))}。同一类内按 Trial 去重，类别之间可能重叠。</p><section id="quality"><h2>任务质量</h2><p>${qualitySummary}分数不抵消硬失败或关键协作问题。</p>${quality}</section><section id="collaboration"><h2>协作诊断</h2><p>按计划 Case × repetition 统计；未知保留分母。三组分布不合成为协作分数。</p>${groups}<h3>逐 Case 变化</h3>${changes.length?changesHtml:'<p class="muted">没有已记录的判定变化，或缺少可比较评价；以评价覆盖与限制为准。</p>'}</section><section id="cases"><h2>Case 与证据</h2><div class="controls"><label>筛选 Case <input data-filter type="search" placeholder="输入 Case ID"></label></div>${details}</section><section id="limits"><h2>版本与限制</h2>${table(['版本','提交','源码摘要'],versions)}<p class="meta">评分 ${escapeHtml(at(assessment,'scoring.version'))} · ${escapeHtml(at(assessment,'scoring.digest'))}<br>方案 revision ${escapeHtml(at(report,'change.revision'))} · ${escapeHtml(report.planDigest)}</p><details><summary>新退化、已有失败与证据缺口</summary>${json({regressions:report.regressions,evidenceGaps:report.evidenceGaps,holdout:report.holdout,limits:report.limits,earlierAttempts:report.earlierAttempts})}</details></section>`)
}

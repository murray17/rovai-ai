...
export const DAILY_ANALYSIS_POLICY = 'daily-health-analysis-v2'
...
输出 JSON：schemaVersion=1、reportId、prepared 提供的 inputDigest、model(provider/snapshotId)、facts/hypotheses/recommendations 数组。每项包含 text、metricPaths（点分路径）、evidenceIds，至少引用一处现有输入。事实覆盖运行状态、工具、A2A 和重要数据缺口；可无假设或建议，不能为了填满而编造。通过 eval:daily analysis 登记；登记验证身份与引用，不证明解释正确。`

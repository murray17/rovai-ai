...
const V090_MIGRATION_SOURCE_PROJECTION_SCHEMA_VERSION: i64 = 44;
const V089_MIGRATION_SOURCE_DATA_CONTRACT_VERSION: &str = "v0.90";
const V089_MIGRATION_SOURCE_PROJECTION_SCHEMA_VERSION: i64 = 43;
...
impl CurrentMigrationState {
    fn admits(&self, contract: &str, schema: i64) -> bool {
        ...
        if contract == V098_MIGRATION_SOURCE_DATA_CONTRACT_VERSION
            && schema == V098_MIGRATION_SOURCE_PROJECTION_SCHEMA_VERSION
        {
            return self.v70
                && self.v71
                && self.v76
        ...
        if contract == V091_MIGRATION_SOURCE_DATA_CONTRACT_VERSION
            && schema == V091_MIGRATION_SOURCE_PROJECTION_SCHEMA_VERSION
        {
            return self.v70
                && self.v71
                && self.v76
        ...
        if contract == V090_MIGRATION_SOURCE_DATA_CONTRACT_VERSION
            && schema == V090_MIGRATION_SOURCE_PROJECTION_SCHEMA_VERSION
        {
            return self.v70
                && self.v71
                && self.v76
        ...
        if contract == V089_MIGRATION_SOURCE_DATA_CONTRACT_VERSION
            && schema == V089_MIGRATION_SOURCE_PROJECTION_SCHEMA_VERSION
        {
            return self.v70
                && self.v71
                && self.v76
        ...
        if contract == V085_MIGRATION_SOURCE_DATA_CONTRACT_VERSION
            && schema == V085_MIGRATION_SOURCE_PROJECTION_SCHEMA_VERSION
        {
            return self.v70
                && self.v71
                && self.v76
...
impl Database {
    ...
    fn migrate_public_a2a_message_delivery_v61(&mut self) -> Result<()> {
        let transaction = self
            .connection
            .transaction_with_behavior(TransactionBehavior::Immediate)?;
        transaction.execute_batch(
            ...
            ALTER TABLE camp_message ADD COLUMN
                ...
                CHECK(json_valid(effective_recipient_ids_json));
            ALTER TABLE camp_message ADD COLUMN
                recipient_set_digest TEXT;
            ALTER TABLE camp_message ADD COLUMN
                recipient_presentation_json TEXT NOT NULL DEFAULT '{}'
                CHECK(json_valid(recipient_presentation_json));
            ALTER TABLE camp_message ADD COLUMN
            ...
            CREATE UNIQUE INDEX camp_message_source_operation_unique
                ON camp_message(source_operation_id)
            ...
            CREATE TABLE message_delivery (
                ...
                recipient_canonical_position INTEGER NOT NULL
                    CHECK(recipient_canonical_position >= 0),
                recipient_digest TEXT NOT NULL CHECK(length(recipient_digest) > 0),
                message_body_digest TEXT NOT NULL CHECK(length(message_body_digest) > 0),
                ...
                ancestor_agent_ids_json TEXT NOT NULL DEFAULT '[]'
                    CHECK(json_valid(ancestor_agent_ids_json)),
                recipient_presentation_snapshot_json TEXT NOT NULL DEFAULT '{}'
                    CHECK(json_valid(recipient_presentation_snapshot_json)),
                ...
                CHECK(
                    ...
                    OR (status = 'interrupted_before_dispatch'
                ...
                CHECK(
                    ...
                    OR (manual_intervention_required = 1
                        AND dispatch_attempt_count = 0)
                )
            );
            ...
            CREATE TABLE message_delivery_attempt (
                ...
                trigger_kind TEXT NOT NULL CHECK(trigger_kind IN (
                    'accepted', 'target_run_ended', 'runtime_ready',
                    'capacity_released', 'explicit_retry'
                )),
                scheduler_correlation_id TEXT NOT NULL,
                status TEXT NOT NULL CHECK(status IN (
                    'attempting', 'waiting', 'materialized', 'failed', 'cancelled'
                )),
                wait_condition TEXT CHECK(wait_condition IN (
                    'target_busy', 'runtime_unavailable', 'capacity_unavailable'
                )),
                ...
                CHECK(
                    (status = 'attempting' AND ended_at IS NULL)
                    OR (status <> 'attempting' AND ended_at IS NOT NULL)
                ),
                CHECK(
                    (status = 'waiting' AND wait_condition IS NOT NULL)
                    OR (status <> 'waiting' AND wait_condition IS NULL)
                )
            );
            ...
            CREATE TABLE message_delivery_retry (
                ...
                reason TEXT,
                created_at TEXT NOT NULL,
                UNIQUE(delivery_id, retry_generation)
            );

            ALTER TABLE agent_run ADD COLUMN
                trigger_message_delivery_id TEXT REFERENCES message_delivery(id);
            CREATE UNIQUE INDEX agent_run_trigger_delivery_unique
                ON agent_run(trigger_message_delivery_id)
        ...
        let migration_result = (|| -> Result<()> {
            ...
            transaction.execute(
                ...
                SET status = 'failed', wait_condition = NULL,
                    failure_code = 'context_formatter_v11_required',
                    failure_detail_json = '{"reason":"collaboration_projection_v2_clean_break"}',
                    ended_at = ?1
            ...
            transaction.execute(
                ...
                UPDATE agent_run
                SET status = 'failed', ended_at = ?1,
                    last_error_code = 'context_formatter_v11_required',
                    wait_reason = NULL, wait_deadline_at = NULL,
                    runtime_recovery_required = 0,
                    execution_lease_owner = NULL,
                    execution_lease_expires_at = NULL,
            ...
            transaction.execute_batch(
                ...
                ALTER TABLE conversation
                    RENAME COLUMN native_member_state_digest
                    TO native_collaboration_state_digest;
    ...
    fn migrate_bounded_omission_evidence_v69(&mut self) -> Result<()> {
        self.connection
            .execute_batch("PRAGMA foreign_keys = OFF;")?;
        let migration_result = (|| -> Result<()> {
            let transaction = self
            ...
            transaction.execute(
                ...
                UPDATE message_delivery_attempt
                SET status = 'failed', wait_condition = NULL,
                    failure_code = 'context_manifest_v9_required',
                    failure_detail_json = '{"reason":"bounded_whole_history_omission_clean_break"}',
                    ended_at = ?1
                WHERE status = 'attempting'
                "#,
                [&now],
            ...
            transaction.execute(
                ...
                UPDATE agent_run
                SET status = 'failed', ended_at = ?1,
                    last_error_code = 'context_manifest_v9_required',
                    wait_reason = NULL, wait_deadline_at = NULL,
                    runtime_recovery_required = 0,
                    execution_lease_owner = NULL,
                    execution_lease_expires_at = NULL,
        ...
        if let Some((table, row_id)) = self
            ...
            .optional()?
        {
            anyhow::bail!("v69 migration left a foreign-key violation in {table} row {row_id}");
        }
        Ok(())
    }
    ...
    fn migrate_composer_recipient_continuation_v85(&mut self) -> Result<()> {
        let transaction = self
            .connection
            .transaction_with_behavior(TransactionBehavior::Immediate)?;
        transaction.execute_batch(
            r#"
            ALTER TABLE camp_composer_draft
            ADD COLUMN continuation_source_message_id TEXT REFERENCES camp_message(id);

            ALTER TABLE camp_composer_draft
            ADD COLUMN continuation_suppressed_source_message_id TEXT REFERENCES camp_message(id);

            ALTER TABLE camp_composer_draft
            ADD COLUMN recipient_selection_touched INTEGER NOT NULL DEFAULT 0
                CHECK(recipient_selection_touched IN (0, 1));

            -- Existing meaningful Drafts predate continuation routing. Keep their route stable
            -- instead of surprising users with a history-derived recipient after upgrade.
            UPDATE camp_composer_draft
            SET recipient_selection_touched = 1
    ...
    fn migrate_compact_context_run_facts_v89(&mut self) -> Result<()> {
        self.connection
            .execute_batch("PRAGMA foreign_keys = OFF;")?;
        let migration_result = (|| -> Result<()> {
            let transaction = self
            ...
            let create_context_manifest_v89 = create_context_manifest
                .replacen(
                    "CREATE TABLE context_manifest",
                    "CREATE TABLE context_manifest_v89",
                    1,
                )
                .replacen(
                    "CREATE TABLE \"context_manifest\"",
                ...
                .replace("run_notice_refs_json", "run_fact_refs_json")
                .replace("run_notice_payload_json", "run_fact_payload_json")
                .replace("run_notice_digest", "run_fact_digest")
                .replacen(
                    "CHECK(formatter_version IN (14, 15, 16))",
                    "CHECK(formatter_version = 17)",
                    1,
                );
            if create_context_manifest_v89 == create_context_manifest
                || !create_context_manifest_v89.contains("CREATE TABLE context_manifest_v89")
                || !create_context_manifest_v89.contains("run_fact_refs_json")
                || !create_context_manifest_v89.contains("run_fact_payload_json")
                || !create_context_manifest_v89.contains("run_fact_digest")
                || create_context_manifest_v89.contains("run_notice_")
                || !create_context_manifest_v89.contains("formatter_version = 17")
            {
                anyhow::bail!("v89 could not rebuild the ContextManifest v15 schema");
            }
            ...
            transaction.execute(
                ...
                SET status = 'cancelled', terminal_source = 'delivery',
                    error_code = 'context_formatter_v17_required',
                    terminal_resolution_source = 'migration',
                    terminal_reason_code = 'compact_context_run_facts_clean_break',
                    version = version + 1, ended_at = ?1, updated_at = ?1
            ...
            transaction.execute(
                ...
                SET status = 'cancelled',
                    cancellation_reason_code = 'context_formatter_v17_required',
            ...
            transaction.execute(
                ...
                SET status = 'failed', wait_condition = NULL,
                    context_manifest_id = NULL,
                    failure_code = 'context_formatter_v17_required',
                    failure_detail_json = '{"reason":"compact_context_run_facts_clean_break"}',
                    ended_at = ?1
                WHERE status = 'attempting'
                "#,
                [&now],
            ...
            transaction.execute(
                ...
                UPDATE agent_run
                SET status = 'failed', ended_at = ?1,
                    last_error_code = 'context_formatter_v17_required',
                    wait_reason = NULL, wait_deadline_at = NULL,
                    runtime_recovery_required = 0,
                    execution_lease_owner = NULL,
                    execution_lease_expires_at = NULL,
            ...
            transaction.execute_batch(&create_context_manifest_v89)?;
            transaction.execute_batch(
                r#"
                DROP INDEX IF EXISTS context_manifest_blob_idx;
                DROP INDEX IF EXISTS context_manifest_bootstrap_idx;
                DROP TABLE context_manifest;
                ALTER TABLE context_manifest_v89 RENAME TO context_manifest;
                CREATE INDEX context_manifest_blob_idx
                    ON context_manifest(rendered_payload_blob_id);
                CREATE INDEX context_manifest_bootstrap_idx
                    ON context_manifest(bootstrap_evidence_id);
    ...
    fn migrate_current_input_skill_links_v91(&mut self) -> Result<()> {
        ...
        let migration_result = (|| -> Result<()> {
            ...
            let create_context_manifest_v91 = create_context_manifest
                .replacen(
                    "CREATE TABLE context_manifest",
                    "CREATE TABLE context_manifest_v91",
                    1,
                )
                .replacen(
                    "CREATE TABLE \"context_manifest\"",
            ...
            if create_context_manifest_v91 == create_context_manifest
                || !create_context_manifest_v91.contains("CREATE TABLE context_manifest_v91")
                || !create_context_manifest_v91
                    .contains("current_input_skill_resolution_json TEXT NOT NULL")
                || !create_context_manifest_v91
                    .contains("current_input_skill_resolution_digest TEXT NOT NULL")
                || !create_context_manifest_v91.contains("formatter_version = 18")
            {
                anyhow::bail!("v91 could not rebuild the ContextManifest v16 schema");
            }

            transaction.execute_batch(
                r#"
                ALTER TABLE agent_run ADD COLUMN
                    skill_selection_snapshot_json TEXT NOT NULL
                    DEFAULT '{"schemaVersion":1,"entries":[]}';
                ALTER TABLE agent_run ADD COLUMN
                    skill_selection_snapshot_digest TEXT NOT NULL
                    DEFAULT 'eaf741c591ae9eb798b55a703ddadfeec7c803b91b3199272a7ccd39e56160c1';
                "#,
            )?;
            transaction.execute(
                r#"
                UPDATE gather_item
                SET status = 'cancelled', terminal_source = 'delivery',
                    error_code = 'context_formatter_v18_required',
                    terminal_resolution_source = 'migration',
                    terminal_reason_code = 'current_input_skill_links_clean_break',
                    version = version + 1, ended_at = ?1, updated_at = ?1
                WHERE status IN ('pending', 'running')
                "#,
                [&now],
            ...
            transaction.execute(
                ...
                UPDATE gather_record
                SET status = 'cancelled',
                    cancellation_reason_code = 'context_formatter_v18_required',
                    version = version + 1, cancelled_at = ?1, updated_at = ?1
                WHERE status IN ('collecting', 'ready', 'completing')
                "#,
                [&now],
            ...
            transaction.execute(
                ...
                SET status = 'failed', wait_condition = NULL,
                    context_manifest_id = NULL,
                    failure_code = 'context_formatter_v18_required',
                    failure_detail_json = '{"reason":"current_input_skill_links_clean_break"}',
                    ended_at = ?1
                WHERE status = 'attempting'
                "#,
                [&now],
            ...
            transaction.execute(
                ...
                UPDATE agent_run
                SET status = 'failed', ended_at = ?1,
                    last_error_code = 'context_formatter_v18_required',
                    wait_reason = NULL, wait_deadline_at = NULL,
                    runtime_recovery_required = 0,
                    execution_lease_owner = NULL,
                    execution_lease_expires_at = NULL,
            ...
            transaction.execute_batch(&create_context_manifest_v91)?;
            transaction.execute_batch(
                r#"
                DROP INDEX IF EXISTS context_manifest_blob_idx;
                DROP INDEX IF EXISTS context_manifest_bootstrap_idx;
                DROP TABLE context_manifest;
                ALTER TABLE context_manifest_v91 RENAME TO context_manifest;
                CREATE INDEX context_manifest_blob_idx
                    ON context_manifest(rendered_payload_blob_id);
                CREATE INDEX context_manifest_bootstrap_idx
                    ON context_manifest(bootstrap_evidence_id);
    ...
    fn migrate_windows_skill_projection_operation_v97(&mut self) -> Result<()> {
        ...
        transaction.execute_batch(
            ...
            UPDATE rovai_data_contract
            SET contract_version = 'v1.15', projection_schema_version = 52,
                reset_reason = NULL, updated_at = datetime('now')
            WHERE singleton = 1;
    ...
    fn migrate_self_authored_recent_messages_v98(&mut self) -> Result<()> {
        self.connection
            .execute_batch("PRAGMA foreign_keys = OFF;")?;
        let migration_result = (|| -> Result<()> {
            let transaction = self
            ...
            let create_context_manifest_v98 = create_context_manifest
                .replacen(
                    "CREATE TABLE context_manifest",
                    "CREATE TABLE context_manifest_v98",
                    1,
                )
                .replacen(
                    "CREATE TABLE \"context_manifest\"",
            ...
            if create_context_manifest_v98 == create_context_manifest
                || !create_context_manifest_v98.contains("CREATE TABLE context_manifest_v98")
                || !create_context_manifest_v98.contains("CHECK(formatter_version = 20)")
                || !create_context_manifest_v98
                    .contains("CHECK(context_delivery_profile_version = 4)")
            {
                anyhow::bail!("v98 could not rebuild the ContextManifest v19 schema");
            }
            ...
            transaction.execute(
                ...
                UPDATE gather_item
                SET status = 'cancelled', terminal_source = 'delivery',
                    error_code = 'context_delivery_profile_v4_required',
                    terminal_resolution_source = 'migration',
                    terminal_reason_code = 'self_authored_recent_messages_clean_break',
                    version = version + 1, ended_at = ?1, updated_at = ?1
                WHERE status IN ('pending', 'running')
                "#,
                [&now],
            ...
            transaction.execute(
                ...
                UPDATE gather_record
                SET status = 'cancelled',
                    cancellation_reason_code = 'context_delivery_profile_v4_required',
                    version = version + 1, cancelled_at = ?1, updated_at = ?1
                WHERE status IN ('collecting', 'ready', 'completing')
                "#,
                [&now],
            ...
            transaction.execute(
                ...
                SET status = 'failed', wait_condition = NULL,
                    context_manifest_id = NULL,
                    failure_code = 'context_delivery_profile_v4_required',
                    failure_detail_json = '{"reason":"self_authored_recent_messages_clean_break"}',
                    ended_at = ?1
                WHERE status = 'attempting'
                "#,
                [&now],
            ...
            transaction.execute(
                ...
                UPDATE agent_run
                SET status = 'failed', ended_at = ?1,
                    last_error_code = 'context_delivery_profile_v4_required',
                    public_runtime_failure_json = NULL,
                    wait_reason = NULL, wait_deadline_at = NULL,
                    runtime_recovery_required = 0,
                    execution_lease_owner = NULL,
            ...
            transaction.execute_batch(&create_context_manifest_v98)?;
            transaction.execute_batch(
                r#"
                DROP INDEX IF EXISTS context_manifest_blob_idx;
                DROP INDEX IF EXISTS context_manifest_bootstrap_idx;
                DROP TABLE context_manifest;
                ALTER TABLE context_manifest_v98 RENAME TO context_manifest;
                CREATE INDEX context_manifest_blob_idx
                    ON context_manifest(rendered_payload_blob_id);
                CREATE INDEX context_manifest_bootstrap_idx
                    ON context_manifest(bootstrap_evidence_id);
...
mod tests {
    ...
    fn current_migration_state_admission_matrix() {
        let supported = [
            ...
            (
                "v1.15/schema-52",
                V098_MIGRATION_SOURCE_DATA_CONTRACT_VERSION,
                V098_MIGRATION_SOURCE_PROJECTION_SCHEMA_VERSION,
                97,
            ),
            (
                "v1.13/schema-51",
    ...
    fn v98_invalidates_profile_v3_context_and_preserves_public_messages() {
            ...
            .send_test_camp_message(
                ...
                &crate::command::CommandEnvelope {
                    ...
                    payload: crate::collaboration::TestCampMessageCommand {
                        ...
                        execution: Some(crate::collaboration::ExecutionRequest {
                            task_id: None,
                            purpose: "Exercise the Profile v4 clean break".to_string(),
                            completion_role: "required".to_string(),
                            budget: None,
                        }),
                    },
        ...
        let (agent_id, conversation_id): (String, String) = database
            .connection()
            .query_row(
                "SELECT conversation.agent_id, conversation.id FROM agent_run JOIN conversation ON conversation.id = agent_run.conversation_id WHERE agent_run.id = ?1",
                [&run_id],
                |row| Ok((row.get(0)?, row.get(1)?)),
            )
            .unwrap();
        ...
        database
            ...
            .execute(
                ...
                SET native_adapter_installation_id = (
                        SELECT default_runtime_installation_id
                        FROM agent_profile WHERE id = conversation.agent_id
                    ),
                    native_session_id = 'native-session-v98',
                    native_binding_compatibility_digest = 'compatibility-v98',
                    native_binding_id = ?2,
                    native_binding_generation = 1,
                    native_binding_secret_digest = 'secret-v98',
                    last_accepted_public_boundary_sequence = 1,
                    native_charter_digest = 'charter-v98',
                    native_collaboration_state_digest = 'collaboration-v98',
                    native_installation_generation = 1,
                    native_session_compatibility_key = 'session-compatibility-v98'
                WHERE id = ?1
                "#,
                params![&conversation_id, Uuid::new_v4().to_string()],
            )
            .unwrap();
        ...
        database
            ...
            .execute(
                ...
                INSERT INTO message_delivery(
                    ...
                    ?1, ?2, ?3, ?4, ?5, 0,
                    'sha256:recipient-v98', 'sha256:message-v98',
                    ?6, 'forward', ?6, NULL, ?6, 1, '[]', '{}',
                    '{"frozenContext":{"formatterVersion":20,"profileVersion":3}}',
        ...
        database
            ...
            .execute(
                ...
                INSERT INTO gather_item(
                    ...
                    status, terminal_source, fallback_summary,
                    fallback_summary_digest, fallback_summary_original_bytes,
                    fallback_summary_truncated, error_code,
                    terminal_resolution_source, terminal_reason_code,
                    version, created_at, started_at, ended_at, updated_at
                ) VALUES (?1, ?2, ?3, 0, NULL, 'pending', NULL,
                          NULL, NULL, NULL, 0, NULL, NULL, NULL,
                          1, ?4, NULL, NULL, ?4)
                "#,
                params![&delivery_id, &gather_id, MUWA_AGENT_ID, &now],
            )
            .unwrap();
        let message_count_before: i64 = database
            .connection()
            .query_row(
                "SELECT COUNT(*) FROM camp_message WHERE camp_id = ?1",
                [&camp_id],
        ...
        database
            ...
            .execute_batch("PRAGMA foreign_keys = OFF;")
            .unwrap();
        let current_manifest_schema: String = database
            .connection()
            .query_row(
                "SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'context_manifest'",
                [],
            ...
            .unwrap();
        let v97_manifest_schema = current_manifest_schema
            .replacen(
                "CREATE TABLE context_manifest",
                "CREATE TABLE context_manifest_v97_test",
                1,
            )
            .replacen(
                "CREATE TABLE \"context_manifest\"",
        ...
        assert!(v97_manifest_schema.contains("CHECK(context_delivery_profile_version = 3)"));
        database
            .connection()
            .execute_batch(&v97_manifest_schema)
            .unwrap();
        database
            .connection()
            .execute_batch(
                ...
                DROP INDEX context_manifest_bootstrap_idx;
                DROP TABLE context_manifest;
                ALTER TABLE context_manifest_v97_test RENAME TO context_manifest;
                CREATE INDEX context_manifest_blob_idx
                    ON context_manifest(rendered_payload_blob_id);
                CREATE INDEX context_manifest_bootstrap_idx
                    ON context_manifest(bootstrap_evidence_id);
        ...
        let reopened = Database::open_with_data_contract_enforcement(&source_directory, true)
            .expect("v1.15/schema-52 should migrate to Profile v4 without quarantine");
        assert!(!source_directory.join("inactive-data-quarantine").exists());
        let contract: (String, i64, i64) = reopened
            .connection()
            .query_row(
                r#"
                SELECT contract_version, projection_schema_version,
                       (SELECT COUNT(*) FROM schema_migration WHERE version = 98)
                FROM rovai_data_contract WHERE singleton = 1
                "#,
                [],
                |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?)),
        ...
        assert_eq!(
            ...
            (
                "failed".to_string(),
                Some("context_delivery_profile_v4_required".to_string()),
            )
        );
        ...
        let delivery: (String, String, i64, String, Option<String>) = reopened
            .connection()
            .query_row(
                r#"
                SELECT status, failure_code, manual_intervention_required,
                       frozen_snapshot_json, context_manifest_id
            ...
            .unwrap();
        assert_eq!(delivery.0, "interrupted_before_dispatch");
        assert_eq!(delivery.1, "context_delivery_profile_v4_required");
        assert_eq!(delivery.2, 1);
        assert_eq!(delivery.3, "{}");
        assert_eq!(delivery.4, None);
        let gather: (
            String,
            Option<String>,
            String,
            ...
            .query_row(
                r#"
                SELECT gather_record.status, gather_record.cancellation_reason_code,
                       gather_item.status, gather_item.error_code,
                       gather_item.terminal_reason_code
                FROM gather_record
                JOIN gather_item ON gather_item.gather_id = gather_record.id
                WHERE gather_record.id = ?1
                "#,
                [&gather_id],
                |row| {
                    Ok((
            ...
            .unwrap();
        assert_eq!(gather.0, "cancelled");
        assert_eq!(
            gather.1.as_deref(),
            Some("context_delivery_profile_v4_required")
        );
        assert_eq!(gather.2, "cancelled");
        assert_eq!(
            gather.3.as_deref(),
            Some("context_delivery_profile_v4_required")
        );
        assert_eq!(
            gather.4.as_deref(),
            Some("self_authored_recent_messages_clean_break")
        );
        let binding: (Option<String>, Option<String>, i64, i64) = reopened
            .connection()
            .query_row(
                r#"
                SELECT native_session_id, native_binding_id,
        ...
        for table in [
            ...
            let count: i64 = reopened
                ...
                .unwrap();
            assert_eq!(count, 0, "{table} should be empty after Migration 98");
        }
        ...
        let reopened_again = Database::open(&source_directory).unwrap();
        let marker_count: i64 = reopened_again
            .connection()
            .query_row(
                "SELECT COUNT(*) FROM schema_migration WHERE version = 98",
                [],
    ...
    fn v97_adds_windows_projection_operation_and_run_registration() {
        ...
        assert_schema_objects(
            ...
            "index",
            &[
                "skill_projection_operation_unique",
    ...
    #[test]
    fn v91_installs_selection_and_resolution_evidence_without_a_legacy_reader() {
        let directory = std::env::temp_dir().join(format!(
        ...
        let current_schema: String = database
            ...
            .unwrap();
        let v90_schema = current_schema
            .replacen(
                "CREATE TABLE context_manifest",
                "CREATE TABLE context_manifest_v90_test",
                1,
            )
            .replacen(
                "CREATE TABLE \"context_manifest\"",
        ...
        assert!(v90_schema.contains("formatter_version = 17"));
        database.connection().execute_batch(&v90_schema).unwrap();
        database
            .connection()
            .execute_batch(
                r#"
                ...
                DROP INDEX context_manifest_bootstrap_idx;
                DROP TABLE context_manifest;
                ALTER TABLE context_manifest_v90_test RENAME TO context_manifest;
                CREATE INDEX context_manifest_blob_idx
                    ON context_manifest(rendered_payload_blob_id);
                CREATE INDEX context_manifest_bootstrap_idx
                    ON context_manifest(bootstrap_evidence_id);
        ...
        let reopened = Database::open(&directory).expect("v90 source should migrate to v91");
    ...
    fn a_new_database_seeds_durable_companions() {
        ...
        assert!(context_columns.contains(&"run_fact_refs_json".to_string()));
        ...
        database
            ...
            .execute_batch(
                ...
                INSERT INTO camp(
                    ...
                    'v85-camp', 'Continuation migration', 'user', 'peer',
                    'quick_chat', '/quick-chat-v85', 0,
                    1, '2026-08-14T00:00:00Z', '2026-08-14T00:00:00Z'
                );
                INSERT INTO camp_composer_draft(
                    camp_id, body, structured_content_json, revision,
                    ...
                    'v85-camp', '保留旧草稿',
                    '[{"kind":"text","text":"保留旧草稿"}]', 5,
                    '2026-08-14T00:00:00Z', '2026-08-14T00:01:00Z',
                    '2026-08-21T00:01:00Z'
                );

                ALTER TABLE camp_composer_draft
                    DROP COLUMN recipient_selection_touched;
                ALTER TABLE camp_composer_draft
                    DROP COLUMN continuation_suppressed_source_message_id;
                ALTER TABLE camp_composer_draft
                    DROP COLUMN continuation_source_message_id;
            ...
            .expect("v84 Draft fixture should be restorable");

        database
            .migrate_composer_recipient_continuation_v85()
            .expect("v85 migration should apply");

        let draft: (String, i64, Option<String>, Option<String>, bool) = database
            .connection()
            .query_row(
                r#"
                SELECT body, revision, continuation_source_message_id,
                       continuation_suppressed_source_message_id,
                       recipient_selection_touched
                FROM camp_composer_draft WHERE camp_id = 'v85-camp'
                "#,
                [],
                |row| {
                    Ok((
            ...
            .unwrap();
        assert_eq!(draft.0, "保留旧草稿");
        assert_eq!(draft.1, 5);
        assert_eq!(draft.2, None);
        assert_eq!(draft.3, None);
        assert!(draft.4);
        let contract: (String, i64) = database
            .connection()

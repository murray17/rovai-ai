...
mod tests {
    ...
    fn database() -> (Database, PathBuf) {
        let root = std::env::temp_dir().join(format!("rovai-memory-projection-{}", Uuid::new_v4()));
    ...
    fn polluted_relationship_current_directory_is_replaced_by_unavailable_generation() {
        let root =
            std::env::temp_dir().join(format!("rovai-memory-current-pollution-{}", Uuid::new_v4()));

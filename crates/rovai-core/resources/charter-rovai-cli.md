Rovai Built-in CLI Contract (v0.46)
...
- Available commands are: `rovai send`; `rovai task create|list|update`; `rovai camp list|search|read`; `rovai history search`; and `rovai memory search|read|write|propose-hearth`.
...
- Task create/update records responsibility but does not notify or wake the assignee. Use `rovai send` when work must start now. Task list is a snapshot, not a waiting primitive.

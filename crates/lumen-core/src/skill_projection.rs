...
const GIT_EXCLUDE_BEGIN: &str = "# BEGIN LUMEN MANAGED SKILL PROJECTIONS";
const GIT_EXCLUDE_END: &str = "# END LUMEN MANAGED SKILL PROJECTIONS";
const MANAGED_TEMP_PREFIX: &str = ".lumen-skill-projection-";
...
fn reconcile_desired_entry(
    ...
    match state {
        ...
        EntryState::Managed(_) => upsert_observation(
            ...
            entry_path,
            "shadowed",
            Some("entry_owned_by_another_lumen_skill"),
        ),
    }
}
...
fn update_git_exclude(execution_root: &Path, managed_entries: &BTreeSet<PathBuf>) -> Result<()> {
    ...
    let mode = fs::metadata(&exclude)
        ...
        .unwrap_or(0o644);
    let temporary = parent.join(format!(".lumen-info-exclude-{}.tmp", Uuid::new_v4()));
...
fn strip_managed_exclude_block(content: &str) -> Result<(String, bool)> {
    ...
    for segment in content.split_inclusive('\n') {
        ...
        if line == GIT_EXCLUDE_BEGIN {
            if inside {
                anyhow::bail!("Git info/exclude contains a nested Lumen managed block");
            }
            inside = true;
            block_count += 1;
            if block_count > 1 {
                anyhow::bail!("Git info/exclude contains multiple Lumen managed blocks");
            }
            continue;
        }
        if line == GIT_EXCLUDE_END {
            if !inside {
                anyhow::bail!("Git info/exclude contains an unmatched Lumen end marker");
            }
            inside = false;
            continue;
        }
    ...
    if inside {
        anyhow::bail!("Git info/exclude contains an unterminated Lumen managed block");
    }
    Ok((output, block_count == 1))
}
...
mod tests {
    ...
    fn projection_uses_minimum_native_roots_and_preserves_git_exclude_content() {
        let root = temporary_directory("lumen-projection-root");
    ...
    fn project_owned_entry_wins_and_lost_observation_is_rebuilt_safely() {
        let root = temporary_directory("lumen-projection-conflict");
        let data = temporary_directory("lumen-projection-db");
        let library_root = temporary_directory("lumen-projection-library");
        let mut database = Database::open(&data).unwrap();
        let library = SkillLibraryService::new(library_root).unwrap();
    ...
    fn imported_delete_drains_projection_then_hard_deletes_content_and_metadata() {
        let root = temporary_directory("lumen-projection-delete");
        let source = temporary_directory("lumen-projection-source");
        let data = temporary_directory("lumen-projection-db");
        let library_root = temporary_directory("lumen-projection-library");
        let mut database = Database::open(&data).unwrap();
        let library = SkillLibraryService::new(library_root).unwrap();
    ...
    fn disabling_waits_for_active_run_before_removing_a_managed_entry() {
        let root = temporary_directory("lumen-projection-drain");
        let data = temporary_directory("lumen-projection-db");
        let library_root = temporary_directory("lumen-projection-library");
        let mut database = Database::open(&data).unwrap();
        let library = SkillLibraryService::new(library_root).unwrap();
    ...
    fn startup_requirements_union_active_camp_member_adapters() {
        let root = temporary_directory("lumen-projection-known-root");
        let data = temporary_directory("lumen-projection-db");
        let library_root = temporary_directory("lumen-projection-library");
        let mut database = Database::open(&data).unwrap();
        let library = SkillLibraryService::new(library_root).unwrap();
    ...
    #[test]
    fn replaced_external_symlink_is_never_removed_as_a_lumen_projection() {
        let root = temporary_directory("lumen-projection-external-link");
        let external = temporary_directory("lumen-projection-external-target");
        let data = temporary_directory("lumen-projection-db");
        let library_root = temporary_directory("lumen-projection-library");
        let mut database = Database::open(&data).unwrap();
        let library = SkillLibraryService::new(library_root).unwrap();

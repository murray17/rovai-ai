...
describe('Feishu identity normalization and passive HTML bootstrap', () => {
  it('chooses the first nonblank typed alias without requiring email or unrelated fields', () => {
    ...
    }, origin, FEISHU_DOMAINS)).toEqual({ brand: 'feishu', userId: 'u_123', userName: '张三 🌼', tenantId: 't_456', tenantName: '示例团队' })
    expect(normalizeFeishuIdentity({
      ...identity, userId: 'ignored', user_id: 'ignored', userName: 'ignored',
      tenantDisplayName: { value: ' ' }, tenant_name: 'ignored', email: ' x@example.org '
    }, origin, FEISHU_DOMAINS)).toEqual({ brand: 'feishu', userId: 'u1', userName: '张三', tenantId: 't1', tenantName: '示例团队', email: 'x@example.org' })
  })

  it.each(['id', 'name', 'tenantId', 'tenantName'] as const)('rejects missing %s without substituting other identity fields', (field) => {
    expect(() => normalizeFeishuIdentity({ ...identity, [field]: '   ', email: 'id@example.org' }, origin, FEISHU_DOMAINS))
  ...
  it('decodes only data literals and a literal JSON.parse, including escaped braces and display names', () => {
    ...
    const result = readOpenPlatformBootstrap(html(`JSON.parse(${JSON.stringify(JSON.stringify(user))})`), `${origin}/app`, FEISHU_DOMAINS)
  ...
  it('never executes arbitrary calls or accessors in the remote page', () => {
    ...
    expect(() => readOpenPlatformBootstrap(source, origin, FEISHU_DOMAINS)).toThrow('feishu_developer_identity_incomplete')
    expect((globalThis as Record<string, unknown>).__feishuFixtureExecuted).toBeUndefined()
    expect(() => readOpenPlatformBootstrap(html('loadUser()'), origin, FEISHU_DOMAINS)).toThrow('feishu_developer_identity_incomplete')
    expect(() => readOpenPlatformBootstrap(html(JSON.stringify(identity), 'fetch("/csrf")'), origin, FEISHU_DOMAINS)).toThrow('feishu_open_platform_bootstrap_incomplete')
  })
  ...
  it('rejects a declared API origin that differs from the final trusted site', () => {
    ...
    expect(() => readOpenPlatformBootstrap(source, origin, FEISHU_DOMAINS)).toThrow('feishu_open_platform_origin_rejected')
  })

  it('reports only missing field names, never raw user or CSRF data', () => {
    try { readOpenPlatformBootstrap(html('{id:"secret-id",email:"private-email"}'), origin, FEISHU_DOMAINS); throw new Error('expected failure') }
    catch (error) {

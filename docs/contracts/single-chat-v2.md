...
v2 不接受外部 Channel Principal 创建或读取 Single Chat，不为 Runtime 原生 delegation、sub-agent、内部模型调用或
...
`open/send/end/pendingInputs.edit` 使用既有 command idempotency。所有 mutation 只允许本地 User，并验证 envelope Camp 与
payload Camp。除 `end` 对同一 exact ended Conversation 的成功 no-op 外，针对既有对话的 mutation 都要求 active
`single_chat` Conversation 与当前 Camp 一致；目标 Agent 必须是 active、未请求离开且 profile `present` 的当前 Camp
...
`singleChat.end` 只接收固定的 `campId + conversationId`，不接收 expected Conversation version 或 active Run ID。Core 在
Command Gateway 的同一事务内按 exact Conversation ID 读取当前实际状态：不存在或 kind 错误返回 `single_chat.not_active`，
Camp 不匹配返回 `single_chat.camp_mismatch`；active 时把 Conversation 标为 ended、取消当前未完成 Run 和未发布 Pending，
并清除 Draft/edit session。消息新增、Agent final 或 Pending 状态变化不构成结束冲突，也不触发先查版本或冲突重试。

第一次结束是线性化点：Conversation version 可以随实际 ended 更新递增，私有 route 立即关闭并拒绝新 send。对同一 exact
Conversation 的后续 `end` 使用新的 commandId 时也返回 `applied/single_chat.ended`，保留当前 version 且不再次更新状态、
取消资源或追加 `single_chat.ended` Domain Event；同一 commandId 继续由 Gateway 重放原结果。迟到
delta/final/tool result/ACK/provider completion 只能成为旧 Run 证据或 cleanup 回执，不得写旧或 successor transcript。
用户可立即创建同一队员的新 Single Chat；不增加跨 Conversation cleanup fence。
...
执行台 narration/plan/command/tool 分组和工具行组件。发送确认前和排队立即显示“连接中”；开始处理但尚未输出时显示“思考中”。
正文、计划、工具或 final 首次出现时，在同一次渲染中移除普通等待提示，后续正文也不在尾部重新追加；活动工具或尚未收口的尾组表达执行进度。只有 Run terminal 才显示耗时总结并自动折叠过程，不能以首段
...
结束确认打开时只冻结 `campId/conversationId/displayName`，不冻结 Conversation version；确认时不得从当前选择重新解析目标。
面板打开或切换对象独占 target selection 与 loading，后台 `singleChat.get` 使用一个 in-flight 标记和一个合并补读标记
串行化同一目标的读取；
`singleChat.list` 只更新 active 列表，轮询和事件刷新不改变目标或 loading。空闲时停止轮询，运行期约 800ms 只读取当前
Conversation；面板收起时使目标请求失效并停止后台刷新。

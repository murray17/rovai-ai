...
| [Runtime Launch and Verification v26（当前）](runtime-launch-and-verification-v26.md) | v25 launch/权限/Cursor 边界不变；ACP matching error 与 Prompt activity 组合确认 accepted input，所有 Runtime 使用安全公开 failure，并分离 AgentRun 审计 wall clock 与 Execution Budget observation |
| [Runtime Launch and Verification v25（历史）](runtime-launch-and-verification-v25.md) | v24 的 Kimi `yolo` 与十二种 Runtime 最高权限默认不变；Cursor 在 Settings 与普通成员 Runtime selector 中保持隐藏，历史配置只读保留；不含 ACP error/activity 与时间域修正 |

...
当前 Camp。`opened_in_system` 只在一次明确用户激活中成功调用默认应用或在系统文件管理器中显示来源已校验的目录后返回，
不创建任何长期资源。消息/工作区的显式绝对路径、Home 相对路径或本机 file URI 直接指向项目外目录时也可系统显示，
不要求或签发 Root Grant；该例外不覆盖相对路径、项目内 symlink 越界、Attachment、Evidence 或预览子资源。
所有文件读取继续执行 containment 校验；目录不能取得读取句柄，不能启动目录包。

消息中的带行号短名只有在同条消息存在唯一明确文件路径时才可复用该来源；请求仍提交原文中的完整来源引用，
行范围只是 Renderer 的阅读目标，不得用拼接出的新路径绕过 Core 来源校验或进行全盘模糊搜索。

消息/工作区路径恰好以单个 `:` 结尾且没有行、列或范围目标时，Main 可在原路径不存在的前提下尝试去掉该冒号。
修正后必须仍为合法文件路径、不产生新位置目标且普通文件实际存在，否则保持原路径；Core 始终校验原始引用。
修正不绕过来源、文件身份或读取根限制，刷新、句柄恢复和系统动作使用相同规则。

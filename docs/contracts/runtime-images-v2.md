...
v2 取代 [v1](runtime-images-v1.md) 的 Runtime 来源规则，补齐真实采样确认的 Antigravity、TRAE、Copilot
图片结果；保存、读取、Renderer、Run 生命周期、Data Contract 和消息/渠道语义保持不变。
...
- TRAE 0.120.52：仅该 Adapter 的 builtin tool update（`_meta.type=builtin`），读取
  `rawOutput.Output.{content,mime_type,file_path}`。明确 `image/*` MIME + base64 content 是图片结果；
  file_path 仅可选元数据，不能独自触发图片。复用同一个 toolCallId accumulator，不在其他 Runtime 猜测此格式。
- Copilot 1.0.79：仅该 Adapter 的 `rawOutput.binaryResultsForLlm[]` 中 `type=image`，
  读取 data/mimeType，复用标准 accumulator、终态和去重；不把一般 rawOutput/content 文字当图片。
- Antigravity 1.1.22：只由当前、已验证 conversation 的 `generate_image` 完成 step 触发。
  stream-json 的 tool_info 只有参数，不能把 ImageName 当结果。通过当前 CLI 专属私有日志的启动端口，
  对固定 loopback 地址调用只读 `GetCascadeTrajectorySteps({cascadeId,stepOffset})`；
  只消费精确 cascadeId/stepIndex、`CORTEX_STEP_TYPE_GENERATE_IMAGE` 和 `CORTEX_STEP_STATUS_DONE`
  对应的 `generateImage.generatedMedia.{inlineData,uri,mimeType}`。空 inlineData 不是 bytes。
  它不是另一个登录、文件扫描、文本解析或远端账号 API；端口/完整响应不公开、不持久化。
  请求两秒有界、返回体有界、禁止重定向/代理/远端 host，失败只略过图片。已有 terminal step 去重避免反复查询，
  写入仍经过当前 Run/epoch fence；停止 Run 不因这次只读补充而获得新的执行或发布权限。
- Cursor 标准 ACP 路径继承通用规则。当前本机旧 CLI 无 ACP，非标准生成通知尚未取得真实 fixture；
  保持未准入，不猜测通知中的建议路径，不擅自升级或改变平台资格。
- Claude 的图片 frame 不复用 2 MiB 文本日志预算。framing 容纳单 Run 图片上限对应的 base64 加协议开销；
  超过有界 frame 预算时丢弃该帧并继续读取独立终态，不能把过大图片变成 AgentRun 失败。

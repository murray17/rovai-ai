...
不存在“暂停队列”或“继续发送”入口；其余输入待新一轮结束后按 FIFO 自动继续。审批、恢复等待等
非终态 Run、未结束的 Delivery 及编辑占用继续阻塞，既有结算或编辑保存/取消后自动解除阻塞。
required Run 失败且该轮实际执行已全部结束时，Turn 正常聚合为 failed；历史 `manual_retry_allowed`
和 `retry_declined_at` 字段不再使已失败的 Turn 等待手动重试。上轮终态为失败或取消时也按同一规则
推进，已公开消息不会退回 Pending 或重复执行。

Core 启动时幂等收敛旧版本仅因该重试标记停留 waiting、且所有 Run 与 Delivery 已结束的 Turn，
保留原失败证据、Pending 内容/顺序与编辑占用，不创建新消息或重试旧 Run。正常 Scheduler 在恢复
完成后按相同准入发布队首；未结束的审批、恢复或执行仍阻塞。

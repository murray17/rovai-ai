...
`<队员名> · 执行中`，终态只显示 `已完成 / 执行失败 / 已取消`；收起状态不显示当前正文、command、
...
展示。每个 AgentRun 有独立过程 disclosure；当前 Run 默认展开、历史 Run 默认收起，用户均可手动切换。连续操作组与每个
有公开 detail 的 command 继续使用与生产执行台相同的嵌套 disclosure，彼此独立展开或收起。页面不提供分页或写操作；
选择某个 Run 只切换顶部触发消息摘要，不替代该 Run 自己的折叠入口。

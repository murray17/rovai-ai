...
- 后续 129 独立移除重复 Evidence 索引并推进 `v1.40 / projection schema 81`；128/129 的历史 marker 均冻结。
- 130 追加 main Fast 偏好生命周期修复，只使资格缓存失效并替换三项触发器，保留三态覆盖与绑定代次；
  它只记录 feature receipt。131 在两侧完整 chain 与新 trigger shape 验证后封闭为 `v1.41 / projection schema 82`。
- 原 main 的 `v1.33 / schema 71 / activity-v2 / through 117` 与
  `v1.34 / schema 72 / activity-v2 / through 118`、`v1.34 / schema 73 / activity-v2 / through 119`
  是三种显式受支持来源。main 的 117/118/119 分别是 Pending/Fast/Fast 生命周期，不可误认成渠道的同号迁移。
...
只在已获 migration ticket 的 exact READ_WRITE/NOFOLLOW、无 CREATE 连接上重映射 main 117→126、118→127、119→130，
保留原 applied_at 和业务行；同一 IMMEDIATE 事务将渠道 marker 退回共享 `v1.29 / schema 70` 起点。
之后运行同一既有渠道迁移链，已映射的 126/127/130 不重建，也不重复失效 Fast 资格缓存；Pending、编辑占用、Fast 选择、Runtime binding revision
与 Usage tier 原样保留。不创建 snapshot/backup，不替换或改名主文件，不改变业务 ID。

126/127/130 可以先于低编号渠道 receipt 存在，因此中间渠道 checkpoint 必须仍可准入和继续；127 缺少 126、130 缺少 127、
128/131 缺少各自完整前置链均拒绝。128 提交前以 schema metadata 复核关键表/列/索引/trigger，按130 receipt区分两种
Fast shape；129 继续自己的唯一约束验证。130 不推进 marker，131 单独封口，避免 main 已映射130而跳过最终合同更新。
...
- 既有 Pending/Fast migration tests 拥有新增 schema、默认值、原配置保留和缺失关键对象时拒绝 contract seal；
  main119的测试作为130的同一合同 owner保留，增加原子失败时触发器/缓存/receipt回滚，不新增平行业务fixture。
- `AuthorityMigrationRunner` 原有完整 fixture 改为原位升级 owner，覆盖旧主线、旧渠道、main Pending、main Fast 与
  main Fast lifetime、joined-128、joined-129 七种来源；同时验证 main 对象不变、业务/credential/Session 保留、
  main119时间戳与资格缓存不被重写、无新 snapshot/manifest/backup。
- 原子失败测试覆盖当前步骤 DDL/receipt 回滚、之前步骤保留与恢复；真实强杀覆盖 receipt 重映射后、126 提交后，以及
  新 main119重映射后、130已提交未封口与旧 snapshot 切换前后两侧。路径漂移和重开期间换库必须拒绝，Lumen 来源不产生 Rovai 空库。

...
本地单聊和执行台共用工具组、工具行、类型/状态 SVG、结果惰性读取和键盘交互。发送确认前及排队显示“连接中”，开始处理但尚未输出时显示“思考中”。
正文、计划、工具或 final 首次出现时立即移除普通等待提示，不延时消失，也不在后续正文下重新追加；活动工具/Compaction 或未收口的尾组表达进度。
审批、重试、网络恢复和停止保留实际状态；非聚焦折叠摘要在已有输出时使用“执行中”，不再称为思考。
...
没有展开内容的静态行不提供整行 hover。可展开 Tool 或 Diff 的 summary 与可点击文件名
按各自动作给出 hover/focus 反馈。
...
可区分路径。只有一个不同文件时显示 `阅读 <文件名或可区分路径>`；多个文件时显示
`阅读 <文件一>，<文件二>`，中文逗号分隔并横向单行排列。动作词固定，空间不足时仅文件名省略；
完整路径保留在 title 与可访问名称中。文件名沿用虚线底线预览入口。展开后继续显示原始完整命令和输出，状态与退出码仍属于整次
...
写入行若有 Diff，动作文字、图标、统计、行内空白与最右侧箭头共用原生可展开摘要，提供
hover/focus 反馈并控制原有 Diff 展开；键盘聚焦摘要后可用 Enter/Space 切换。点击文件名不得切换
Diff，点击摘要其他区域不得打开文件。没有 Diff 时仅文件名可点击。缺少可靠路径时不生成文件链接；缺少 Diff 时不生成空展开入口。
...
- 四条纯 `sed -n` 涉及两个不同文件时仍是一个命令项并显示 `阅读 <文件一>，<文件二>` 的单行摘要；重复读取同一文件只列一次，
  同名不同路径可区分，混合或未支持语法保持原 Shell 展示；
- read 行不可展开；新增、编辑、path-only write 和无可靠 path／Diff 的回退符合上述规则；
- 文件名预览与 Diff 摘要可独立鼠标和键盘操作，失败只产生红色 Toast 且不改变已有预览状态；

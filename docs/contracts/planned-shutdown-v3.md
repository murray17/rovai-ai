...
Renderer 收到 `runtime.state = shutting_down` 后显示无操作按钮的 modal：标题为“正在取消所有 AgentRun”，
说明 Rovai 将停止所有运行、完成本地收口后退出，并明确未确认的文件、命令或工具效果会保留为待核对记录。
modal 必须可聚焦、标记为 modal dialog，并在 reduced-motion 下停用 indeterminate motion。

...
# DingTalk Channel v1 Contract

本合同拥有钉钉账号、DWS Gateway、队员应用发布、Stream 入站、Owner identity、Provider-specific snapshot 和外部投递
...
`ExternalPrincipal` 无本机 Owner 权限。Renderer 不能接收 OAuth Token、refresh token、AppSecret、DWS stdout/stderr、
profile selector、credential payload 或 Host-only routing facts。Core 只保存 identity digest、显示字段、远端非秘密 identity
和 `credentialRef`。Main 用 OS `safeStorage` 保存每 App `{appKey, appSecret, robotCode}`；加密不可用时 fail closed。

Helper 必须同时满足：DWS version `1.0.60`；目标平台 binary SHA 与随包 allowlist 一致；`shell=false`；隔离
`DWS_CONFIG_DIR`；固定 operation/argument；默认 120 秒、OAuth 10 分钟 timeout；AbortSignal 可取消；mutation 显式
...
```text
auth.login / auth.logout / auth.status / profile.switch
...
`auth.status` 只有同时返回 `success=true`、`authenticated=true`、`token_valid=true` 以及完整
`corp_id/user_id/user_name/corp_name` 时才能投影 connected identity。

切换事务顺序：inspect old → OAuth new → inspect complete new → Core upsert → commit current。取消、timeout 或新登录失败
不改旧 Profile；Core upsert 失败且 identity 已改变时必须执行 `profile switch <oldCorpId>:<oldUserId>`。切回不登出新 Profile。
断开只 `auth logout --profile <exactCorpId>:<exactUserId>` 并 CAS 更新 Core account；不删除 Bot credential 或远端 App。
...
`provisioningMode` 只能是 `dws_gateway`。远端 facts 为
...
| helper 缺失/hash 不符/平台不支持 | `dingtalk_dws_unavailable` / `dingtalk_dws_integrity_failed` / `dingtalk_dws_platform_unsupported` |
...
Feishu/DingTalk UNION view。随后写入 `Data Contract v1.35 / projection schema 76` 与 Migration 122。

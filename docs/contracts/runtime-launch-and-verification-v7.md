...
# Runtime Launch and Verification v7

本合同继承 [v6](runtime-launch-and-verification-v6.md) 的 launch purpose、light discovery、显式检查、
execution-deferred verification、权限默认、Prompt fence、response-only input ACK、TRAE 冷 Session 恢复
和 replay quarantine，并收紧恢复 response 的 exact-ID 边界。
...
在 replay barrier 前不得发送当前 AgentRun prompt。`session/load` 或 `session/resume` 的成功 response
可以省略 `sessionId`，也可以返回与请求完全相同的 ID；两种情况都继续使用原始 exact Session ID。
如果 response 返回不同 ID，必须把恢复判定为协议异常、标记 Host protocol-violated 并进入失败回退。
不得解绑原始 ID 后绑定返回 ID，也不得把返回 ID 写入 Runtime、Conversation Native Binding 或后续 prompt。
...
JSON、目标之外的 Session event、恢复 response ID 不一致、其他协议异常、Host exit、timeout 或 error
response 都必须拒绝恢复、标记 Host protocol-violated 并进入失败回退。恢复期 server request 可以收到
通用拒绝 response，但其请求和结果不能投影为当前 Run 的 Approval/Action。
...
错误 ID、恢复 response ID 不一致、workspace/模型/权限/Host/executable 不兼容、load timeout 或 replay
异常都不得静默保留旧 Binding。当前 prompt 已发送后的不确定性继续受 Accepted Input Recovery 合同约束，
不使用本节重发路径。

## 6. Unchanged boundaries

- v6 的 TRAE light discovery、Availability Check、launch matrix 与 Ready commit fence 不变；

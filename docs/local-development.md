...
- `smoke:memory` 在隔离数据目录中验证 Memory Migration/重启、自动形成策略默认开启与切换、用户治理、幂等、Secret 拒绝、Revision/Supersession/Forget、Export v2、Projection v2 污染恢复、文件权限和无正文诊断；不调用模型。Native Binding 下的非家园自动新增、每 Run/每作用域额度与可选确认由 Rust 集成测试覆盖。

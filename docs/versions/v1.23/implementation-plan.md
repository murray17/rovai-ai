...
# v1.23 Windows x64 本机实现与 Runtime 复核计划

## 1. 最新 `main` 集成

- [x] 在可恢复的 `codex/windows-x64-adaptation` 分支保存 Windows 改动；
- [x] 获取并合入 `origin/main` 到 v1.22 delivery（`ef2eab5`），不使用 reset/stash 覆盖本地工作；
- [x] 按当前 Probe supersession、Attachment View、Runtime Files Root、Built-in Transport 与 ACP metadata 语义解决重叠；

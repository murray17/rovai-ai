...
# Rovai-ai v1.30：Pi Coding Agent Runtime 接入
...
- 保留 `AdapterKind=pi`、`pi-jsonl-rpc-v1`、Runtime Activity descriptor、Renderer 目录和 macOS arm64
  admission 实现；主线 Migration 109 保留 Runtime entrypoint locator identity，Migration 110 增加 Camp 动态成员，
  Pi Migration 111 增加 catalog，Migration 112 增加 managed context，最终 Data Contract 为
  `v1.25 / schema 66 / migration 112`；
...
Migration 112 fence 所有缺少 frozen identity/managed receipt 的旧非终态 Pi technical state，使用稳定
...
- Rust 回归覆盖 workspace resident reuse、member/camp invalidation 不淘汰 Pi Host、Bootstrap identity freeze、
  accepted receipt gate、无 Pi redelivery overlay，以及主线 Migration 109、Camp Migration 110 后的 Pi
  Migration 111/112；

...
>
> 前置版本：[v1.23 按需 Built-in CLI Help 与 Charter 精简](../v1.23/README.md)已按完成事实冻结为 historical。

## 版本目标

在不回退 v1.23 Built-in Tool Transport v20、v1.22 Runtime Probe supersession、v1.21 User Automation 与
v1.20 Camp Published Attachment View 的前提下，把 Windows x64 实现迁到最新 `main`，完成 native build、
NSIS、外置 legal payload、隔离安装、planned shutdown 和十个 Runtime 的真实本机验收；开发态本机成功与
发布平台资格必须保持不同证据层级。

## 交付范围

- 合入 `origin/main@645994cc`，保留 v1.23 按需 CLI Help、Charter revision fence、Built-in Transport v20、
  Probe supersession、Attachment View、Runtime Files Root 与 User Automation；
...
Camp Attachment View、User Automation 或 Built-in Transport v20 wire shape。Windows backend 和验收脚本使用 v1.23
...
| 范围 | 结论 | 证据或理由 |
| --- | --- | --- |
| Version lifecycle | 已更新 | v1.23 冻结为 historical；本概览、实施计划与版本索引建立唯一 current v1.24。 |
| Decisions | 确认无需更新 | Windows 长期取舍继续由 v1.05/v1.15 决定和当前 Architecture/Contracts 拥有；本版不改变 v1.23 Charter/Transport 决定。 |
| Contracts | 确认无需更新 | 不改变 Runtime Launch v17、Runtime Platform Admission、Managed Process、Private Storage、Planned Shutdown、Attachment View 或 Built-in Transport v20 字段/wire/error 语义。 |
...
| Runtime compatibility | 已更新 | [兼容性清单](../../runtime-compatibility.md)记录本机版本、Flash 边界、CodeBuddy ACP 差异、Windows handle 边界与 Built-in v20 开发态矩阵，同时保持正式 Windows admission 为 `not_qualified`。 |
| Documentation routing | 已更新 | 版本索引路由到 v1.24；[Windows packaging guide](../../development/packaging-windows.md)记录已实现命令、legal payload 与发布边界。 |
| Root README | 确认无需更新 | Windows 11、正式签名和逐 Runtime immutable qualification 未完成，根 README 不提前声明常青 Windows 支持。 |

## References

- [v1.24 实施与验收计划](implementation-plan.md)

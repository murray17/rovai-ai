...
# v1.28 Pi resident Runtime revision 1 实施验收计划
...
- [x] Data Contract 升级为 `v1.22 / schema 63 / migration 108`；
...
- [x] 真实 stdio fixture 已通过 initialize/list/call；Streamable HTTP 保持 adapter unsupported。
...
- [x] Migration 108 只从 v1.21/schema 62/migration 107 升级，新增 Evidence v2/receipt table/acceptance trigger；
...
- [x] 旧 migration synthetic fixture 已适配 schema 63 约束，并保留逐版本验证。

## 9. 自动化与收尾

- [x] Pi adapter capability、workspace Fleet LRU、Bootstrap Evidence/receipt、Migration 108 与 MCP bridge 定向
  tests 通过；
...
- [x] `cargo fmt --check`、Clippy、Core 全量 tests 与 `git diff --check` 最终通过；
- [x] `pnpm typecheck`、523 项 Vitest、完整 `pnpm test`、`pnpm docs:test` 与 `pnpm docs:check` 最终通过。
...
- [x] 区分 stdio MCP 已支持、Streamable HTTP 未支持，以及 fixture/真实 Runtime/未资格化能力的证据层次；

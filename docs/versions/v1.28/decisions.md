...
`grok-build × macos-arm64` 与 `grok-build × macos-x64` 分别绑定本版本各自独立的 evidence digest；
Windows x64 保持 `not_qualified`。Usage/Cost 在语义未验证前保持 Disabled。本决定同时取代 v1.27 冻结时的 Kimi
...
使用其 load-only HistoryRestore。macOS arm64 与 macOS x64 已分别完成目标主机取证，Windows x64 仍待独立
取证；版本门共享不代表平台验证可以互相外推。

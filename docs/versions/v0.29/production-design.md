...
“队员”标题左侧提供 `28×28px` 小箭头按钮，Tooltip 与无障碍名称统一为“返回首页”；
点击固定返回 Quick Chat 首页。该入口与其他离页入口共享单队员 Runtime dirty 草稿保护，
不绕过“继续编辑 / 放弃更改”。

队员页主内容不再渲染独立名册栏，只显示当前选中队员的详情。侧栏与主内容之间只有
一份名册选择状态，不允许用隐藏的第二份列表维持旧 Workbench。
右侧详情表面使用与 Quick Chat 首页一致的白色 `--home-surface`；统一侧栏仍使用 Rail
表面，不随右侧背景变化。
...
右侧详情 Header 的圆形 icon 保持纯展示；身份页右侧半身照是“更换角色图片”入口，点击
或键盘激活后复用现有图片裁剪 Dialog，并在关闭后把焦点返回半身照。半身照到身份区
上下分隔线使用对称留白。

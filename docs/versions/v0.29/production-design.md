...
队员名册块顶部提供与设置页一致的“返回 App”入口，返回进入队员页之前的一级页面；
该入口与其他离页入口共享单队员 Runtime dirty 草稿保护，不绕过“继续编辑 / 放弃更改”。
...
右侧详情 Header 的圆形 icon 本身是“更换角色图片”入口，点击或键盘激活后复用现有图片
裁剪 Dialog，并在关闭后把焦点返回头像按钮；Header 内容到上下分隔线使用对称留白。

...
- [ ] 合并后通过 Rust fmt、TypeScript、Node/Vitest、文档和 diff 门禁。
...
- [ ] 在最新 `main` 合并结果上重新执行 build、NSIS、verifier、installer lifecycle 和 planned shutdown；
...
- [ ] 在最新 `main` 合并结果上重新执行十 Runtime 全矩阵；
...
- [ ] 删除临时登录 helper、失败 Fixture 和已恢复的 sidecar backup，保留非敏感验收报告；
- [ ] 确认用户 API key 未进入 Git diff、日志、报告或诊断 bundle；
- [ ] 全量复跑通过后将本版状态更新为 implemented；不执行关机。

...
评分升级不覆盖旧报告，也不把旧 Judge 判定直接换算为新分数。需要比较时，在同一标准下重新评价两侧保留证据；缺少所需证据则重新执行。真实 Judge 与完整新旧对照仍须实际运行后记录，未运行不得写成通过。既有 macOS 受管 Runtime 中嵌套 Runner 受阻的历史事实保持；后续按用户确认的防误调用边界移除 Runtime 外层沙箱，见 [V1.58-D05](decisions.md#v1-58-d05)，不重写旧评测结果。

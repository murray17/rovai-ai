...
- [x] `singleChat.end` 只接收 exact `campId + conversationId`，不做 expected version CAS；active 状态在同一 command transaction 内结束并清理，
  已 ended 的同一 Conversation 成功 no-op，不重复推进 version、取消资源或写 ended Domain Event。
...
- [x] Renderer/Preload/Main bridge 接入 Source Ref Draft 与 Pending 编辑，Desktop Main allowlist 包含
  `singleChat.pendingInputs.edit`；复用公共拖拽识别、AttachmentCard、Preview、Open、Reveal 和 FilePreview。文件选择、
  拖拽与粘贴临时文件进入同一 Source Ref 流程，运行中 Composer 可继续发送，空 Draft 仍显示停止动作；对象切换使用唯一目标请求
  序号和当前队员双重 fence，结束确认只冻结 Camp、exact Conversation ID 与显示名。
- [x] Single Chat panel 改为打开时一次 list + get、空闲时不轮询；运行期约 800ms 只 get 当前 Conversation，terminal 且无
  `queued` Pending 后立即停止；`refreshList` 只更新列表，`refreshCurrent` 以 in-flight/补读两个标记合并轮询、事件与 mutation，
  后台刷新不修改目标或 loading，panel 收起后不保留刷新计时器。
...
| Gate | 状态 | 证据 |
| --- | --- | --- |
| Single Chat Core 定向回归 | `passed` | `single_chat::tests` 8 项通过；覆盖 exact-ID 结束/重复结束 no-op、私有 final/迟到 fence、Source Ref 消费与 Run 解析、History、FIFO repair 和 Skill policy |
| Migration 140 原子性 | `passed` | v1.49/schema 90 准入、receipt failure 回滚、旧私聊附件表缺失与 v1.50 current marker 定向测试通过 |
| TypeScript 与 Renderer 回归 | `passed` | `pnpm typecheck` 通过；Vitest 152 个文件、1537 项测试通过，其中 SingleChatPanel 定向回归 12 项，覆盖固定结束目标、刷新职责与串行合并标记 |
| 原生 Single Chat UI | `platform-blocked` | fixture 已接入共享 Electron sandbox admission；当前嵌套 macOS sandbox 被规范化识别并跳过，不把未运行的原生断言记作通过 |
| Rust 总门禁 | `passed` | workspace/all-targets check、CLI 33 项、Core binary 219 项和适用的 Core lib 505 项测试通过；Core lib 总计 506 项中，唯一未通过项仍是未改动的 macOS sandbox-exec 测试，当前嵌套宿主返回 exit 71 而按现有规则定向排除 |

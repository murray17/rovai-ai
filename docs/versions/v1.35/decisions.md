...
<a id="v1-35-d01"></a>
## V1.35-D01：项目绑定是 Owner 本机目录，不把外部成员提升为本地用户

> 当前产品语义已由 [V1.35-D09](#v1-35-d09)取代；本节保留早期方案的取舍记录。
...
<a id="v1-35-d02"></a>
## V1.35-D02：多 Bot 先聚合、再走统一原子 admission 与单根 FIFO
...
<a id="v1-35-d03"></a>
## V1.35-D03：飞书 reply 永远冻结为当前消息的 ExternalQuote
...
<a id="v1-35-d04"></a>
## V1.35-D04：群 Bot roster 复用 Camp Membership v1，普通群与话题采用不同扩张规则

> D04 关于完整 roster authority、普通群同步和复用 Camp Membership v1 的决定继续有效；“话题仅按 mention/A2A
> 按需扩张”的部分已由 [V1.35-D15](#v1-35-d15)取代。
...
<a id="v1-35-d05"></a>
## V1.35-D05：秘密与网络留在 Main，公开结果由 Core Outbox 可靠投影
...
Developer Session 与 App provisioning 的进一步边界由 [V1.35-D06](#v1-35-d06)修正；本决定继续拥有
...
<a id="v1-35-d06"></a>
## V1.35-D06：Developer Session 与队员 App credential 分离，普通发布直连开放平台控制台
...
[V1.35-D08](#v1-35-d08)进一步收敛。Manifest 只保留头像和兼容元数据，不参与运行时 readiness
...
<a id="v1-35-d07"></a>
## V1.35-D07：飞书 Bot 复用队员受控头像，并允许冻结 App 做同身份修复
...
<a id="v1-35-d08"></a>
## V1.35-D08：队员 App 采用 template-first、durable freeze 与 activation-first 收敛
...
<a id="v1-35-d09"></a>
## V1.35-D09：飞书一期收敛为 Owner-only Camp，群与话题首次在私聊冻结项目

> Owner-only、Quick Chat、不可换绑与统一 admission 仍有效；项目卡投递位置和完成态已由
> [V1.35-D12](#v1-35-d12)取代。本节保留此前私聊方案的取舍记录。

### 背景

V1.35-D01 允许绑定会话中的任意飞书成员触发 Agent，并让 Owner 在渠道设置页维护第二套 ProjectBinding 目录与会话
...
<a id="v1-35-d10"></a>
## V1.35-D10：发布期解析并冻结每个 App 的 Owner open ID
...
<a id="v1-35-d11"></a>
## V1.35-D11：执行过程使用临时控制台，公开正文与附件保持永久且独立
...
<a id="v1-35-d12"></a>
## V1.35-D12：项目选择回到原会话，Core 先失权再异步撤回
...
<a id="v1-35-d13"></a>
## V1.35-D13：终态执行台在原卡持久收起，Owner 以 CAS 展开完整过程

> D13 的终态收起、持久 view state、nonce/CAS 与 expand/collapse action 已由
> [V1.35-D16](#v1-35-d16)取代；per-AgentRun 原卡、同卡分页和下一根请求 recall 边界继续有效。
...
<a id="v1-35-d14"></a>
## V1.35-D14：飞书执行卡只显示安全操作摘要，命令输入输出留在本机

> D14 关于不展示工具输入输出、Secret 和完整 patch body 的边界继续有效；只显示“执行命令”、expand/collapse 与持久
> CAS callback 的部分已由 [V1.35-D16](#v1-35-d16)取代。
...
<a id="v1-35-d15"></a>
## V1.35-D15：话题群当前 Rovai Bot roster 是 Topic Camp 的动态默认协作队员池
...
<a id="v1-35-d16"></a>
## V1.35-D16：终态执行卡使用 sealed snapshot、同源安全命令与无状态分页

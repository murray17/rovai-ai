...
# Rovai-ai v1.35：飞书队员 Bot 与 Camp 渠道
...
前置主线版本：[v1.34](../v1.34/README.md)。本目录由渠道分支原 v1.33 顺延为 v1.35，冻结原实施与验收事实。
...
| Decisions | 已更新 | [v1.35 决定](decisions.md#v1-35-d16)冻结 Owner-only Camp、Quick Chat、原会话项目卡/异步撤回、聚合/统一 admission、ExternalQuote、动态 Topic roster、template/activation-first Provisioner、App-scoped Owner prebinding，以及临时执行控制台的 terminal sealing/同源安全命令/无状态分页与永久输出/原生附件边界。 |
...
## 历史勘误：2026-08-30 主线版本编号冲突

主线 `4e796bde` 使用 v1.32 记录外部附件快照。为保留主线编号，渠道 `d87e88b6` 的本记录从 v1.32
顺延到 v1.33；只修改版本元数据、决定 ID/锚点和文档链接。此前 v1.30 → v1.32 的迁移说明仍是当时事实，
历史取舍、Data Contract 编号、模型确认 revision 和原验收判断没有改写，可按两个 Git revision 对照。

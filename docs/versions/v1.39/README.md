...
- 正式 Host 使用 `pi --mode rpc --no-themes --extension <rovai-pi-host-v5>`，保留 Pi 原生 Built-in tools、
  Extensions、Skills、Context files、Prompt templates 与用户 Settings。自动发现 Extension 阻断 pre-input RPC 时只
  降级重试一次 `--no-extensions`；Rovai extension 仍显式加载，Machine Ready 继续使用 managed-only 隔离环境。
...
- Bootstrap 通过官方 extension `before_agent_start` 在自身 hook 位置进入高权限 system prompt；后加载 Pi extension
  仍可能继续改写最终 provider payload。每轮不可变 V2 managed-input receipt 只证明 exact binding/session/cwd、
  Bootstrap 与 `bash/edit/write` 三个 governed Tool 可观察，并与 input acceptance 原子提交；不再声称完整环境 attestation。
- Rovai managed Skill root 作为 Pi 原生 Skill 的追加来源，Core 只验证本轮分配项是 catalog 子集。direct human
  prompt/skill slash command 由 Core 按 `get_commands` 显式展开并私有留证；当前已授权 PNG/JPEG/GIF/WebP 以 exact
  bytes/MIME/order 发送到 `prompt.images`，零图片和有图片都保存私有集合摘要。
...
- Pi 原生没有 sandbox 或 permission popup；Rovai 的 `partial_managed` 只在 `bash/edit/write` 执行前提供 Durable
  Approval，只读与未知 Extension Tool 按 Pi 自身语义执行。它不覆盖全部 mutation，也不证明后加载 handler 未改写
  input。Shell Action 保存 Pi 在当前 project trust 下实际解析出的 shell path、args 与 command transport，不伪造成
  `/bin/zsh -lc`。
...
- Pi Prompt images 已接入原生 RPC；结构化 Web Search 与 Camp Fast 没有可靠 Pi 0.84.4 evidence，保持明确
  unsupported/hidden，不从正文、path、MCP 名或普通 query 猜测。
...
Migration 135 只接受 `Data Contract v1.44 / Projection Schema 85`，原子升级到 v1.45/schema86，并创建
Pi receipt 基础表与 guard。Migration 136 只接受 v1.45/schema86，原子升级到 `Data Contract v1.46 / Projection
Schema 87`：receipt reader 接受历史 V1 与当前 V2、Writer 只写 V2；新增 Pi Prompt Transform 与图片私有证据表，
把既有 Pi `approval_mode=managed` 规范化为 `partial_managed`。Receipt 直接 UPDATE/DELETE 保护与父 Delivery 合法
`ON DELETE CASCADE` 并存；失败整体回滚，提交后 `foreign_key_check=0`，重开不会重复应用。

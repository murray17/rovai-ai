...
发布复用现有 SQLite 消息内核，成功结果与 Pending 同事务保存。发布失败保留为 needs_repair，
用户通过既有编辑入口保存或删除，不增加重试按钮。

按用户在实现期间确认的交互，Composer Stop 完全停止当前执行后自动发出一个队首，后续仍按轮
执行；不要求再点击继续，也不增加单步队列模式。根据用户验收反馈，彻底删除队列暂停/继续状态、
接口和入口；已有非终态检查负责等待 Run/Delivery 完全结束，结算后自动推进下一条。
发送按钮统一为“发送”，不随入队或提交状态更换文字。编辑仅从独立小铅笔按钮进入，行背景与正文没有编辑点击行为。
...
- 独立暂停/继续队列：与上一轮结束就发送下一条的交互相冲突，且可能遗留与执行状态脱节的暂停；

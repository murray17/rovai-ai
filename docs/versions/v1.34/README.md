...
# Rovai-ai v1.34：钉钉队员 Bot 与 Camp 渠道

> 当前合并状态：飞书终态双层折叠与分页修复、主线外部附件快照已合入。钉钉包含
> `fb9b0042` 的 Web Session WIP checkpoint，完整 Bot 发布未验收。控制台凭据及配置/权限/事件/版本/发布协议仍待完成，
> `67010` 的权威原因未确认；Cookie/重启证据只来自隔离 Probe，不是 packaged App 验收。
>
> 下文 OAuth 范围保留此前实现基线，不代表 Web Session 已取得生产资格；当前实现与旧钉钉合同的差异、
> 待核验边界见[研究记录](../../research/dingtalk-web-session-probe.md)。本次合并和打包不关闭钉钉 NO-GO。

前置版本：[v1.33 飞书队员 Bot 与 Camp 渠道](../v1.33/README.md)已按完成事实转为 historical。
...
| 范围 | 结论 | 证据或理由 |
| --- | --- | --- |
| Version lifecycle | 已更新 | 本概览、[实施计划](implementation-plan.md)、[决定](decisions.md)与[版本索引](../README.md)共同切换 `current_version`；主线 v1.32 与迁入的飞书 v1.33 保持 historical，v1.34 为唯一 current。编号迁移不改历史取舍、Data Contract 或验收结论。 |
| Decisions | 已更新 | [v1.34 决定](decisions.md)记录 Main 直接拥有 OAuth/Developer API、provider-neutral Core 复用和未实测能力 fail-closed 的高成本取舍。 |

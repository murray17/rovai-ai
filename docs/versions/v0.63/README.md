...
- MCP chooser 提供名称、Endpoint、Transport 与来源搜索，以及全部、已分配、未分配筛选；
  checkbox 即时保存，筛选结果可以批量选择或清空；
- 高权限 MCP 继续逐项进入现有确认 Dialog，批量选择不能静默授权；批量变更按 Core 返回的最新
  `configDigest` 串行提交，保留既有 CAS 冲突恢复；
- 已安装 MCP 使用稳定 `serverId` 身份色 mark 和开放列表行，展示来源、Endpoint、真实队员头像
  摘要、启停与详情；编辑 JSON 和删除收进展开详情，不再形成意义不明的常驻操作栏；
- 保留标准 JSON 真源、Finder 入口、malformed fail-closed、权限修复、显式导入、风险确认与

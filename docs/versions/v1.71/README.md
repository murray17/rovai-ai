...
- Renderer 增加 Lark 页签与未验收提示；自动化通知可以选择 Lark。
...
v1.67 到 v1.70，并用掉 Migration 171 到 173。本版先后两次改号，最终为 v1.71：Lark 迁移为 Migration 174，数据合同
为 v1.71 / schema 124。实施计划中的 S1 到 S4 记录保留原基线上的证据，每次改号后的门禁结果另行记录。
...
- 编号：上游作者已在 Issue #523 确认独立 provider 的边界，并约定版本号顺位继承、合并冲突时再处理。#517 先合入并占用
  v1.70 与 Migration 173，本版因此改号为 v1.71 与 Migration 174。
...
| UI | 已更新 | [渠道设置](../../ui/components/channel-settings.md)增加 Lark 页签、品牌显示与未验收提示 |

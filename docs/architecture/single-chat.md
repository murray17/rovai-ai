...
| 组件 | 拥有 | 不拥有 |
| --- | --- | --- |
| Desktop Renderer | active 会话选择、私有 transcript、执行折叠、停止与结束意图 | 权限、路由、恢复推断、公共水位 |
| Desktop Main bridge | 五个 typed Core method 的 allowlist 和事件转发 | 业务状态、目标选择、私有输出生成 |

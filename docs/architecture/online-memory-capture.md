...
[ADR-0180](../adr/0180-single-agent-memory-write-command.md)；精确字段与状态见
[Memory Capture v1](../contracts/memory-capture-v1.md)，CLI transport 见
...
current input / A2A handoff / explicit long-term commitment / verified result
            ...
            Runtime-native memory-stewardship discovery
                   ...
                   search -> read -> decide
                    ...
                    rovai memory write
                              |
          CLI v9 + Core BuiltinToolRouter + active lease
                              |
                              v
...
| Built-in Tool Runtime | `memory.search/read/write` | CLI parsing、active lease、Envelope/projection/replay transport | Memory semantics、review lifecycle、candidate visibility |
| Agent Memory Capture Facade | one validated add/revise request | actor resolution、common canonicalization、安全/配额 admission、route selection | semantic similarity、user review decision |
...
| Memory Retrieval Broker | search/read | live applicable set、current Revision、cache state、anti-oracle | pending Review candidates、historical body fallback |
| User Memory Governance | typed user commands | all Scope writes、mutual、review decision、lifecycle、Forget、Supersession | model-inferred intent or optimistic Renderer truth |
...
untrusted; direct read continues from authoritative rows with current authorization. Pending candidate existence is
not discoverable through ranking, snippets, result counts or guessed IDs.

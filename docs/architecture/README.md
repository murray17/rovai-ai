...
| [飞书渠道](feishu-channel.md) | Developer Session/队员 Provisioner、Owner-only 入站、Quick Chat/PendingCampBinding、ExternalPrincipal、多 Bot 聚合、统一 Camp admission、群 roster、状态执行卡、LAN 只读执行台、永久输出/附件及 Main secret/Core Outbox 边界 |
| [钉钉渠道](dingtalk-channel.md) | Main Web Session/Console API/Stream、独立队员应用机器人、Owner-only 私聊/群聊、provider-neutral admission、群 roster、Quick Chat、三入口状态卡、共享 LAN 执行台、Markdown、feature gate、共享 credential/Session 持久化与 Main secret/Core Outbox 边界 |
...
| [File Preview](file-preview.md) | Core 领域来源、Main 具体文件能力/窗口句柄/重开、分页读取、显式目录 Root Grant、root watcher、HTML 文档目录协议与资源生命周期 |

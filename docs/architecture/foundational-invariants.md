...
- Pi 使用独立 JSONL RPC、v5 薄 managed extension 与统一 Fleet。正式 Host 保留 Pi 原生 Built-in tools、Extensions、Skills、Context files、Prompt templates 与用户 Settings；Rovai 不重建完整环境。Pi Resident Host 可以串行切换多个 Session，但一次只能拥有一个 Run；其复用 identity 是 canonical workspace + process digest，当前 lease 的 Camp/member 只用于精确失效并随领取更新，其他 Runtime 的 member-scoped identity 不变。Session、Bootstrap、Skills、model 和 Prompt 都是 bind/session 输入而不是 process LRU key；MCP Assignment 与配置完全不参与 Pi compatibility、复用、恢复或 LRU。Runtime 创建按 `(agent_run_id, execution_epoch)` singleflight，registry 锁不得跨 IO。恢复优先实际 `switch_session` 到 Core 私有的完整 canonical file，并由 `get_state` 同时核对 full Session ID、file 与 cwd；locator 不可用时记录 continuity lost 后最多创建一个新 Session，不把 exact resume 当作可用性硬前提。完整 locator 不进入任何公开事件、Activity、diagnostic 或 read model。Machine Ready Probe 必须用临时 `--session-dir` 与 private `--session` seed 初始化空 Session，全程禁止 Prompt、Tool、MCP 和 Provider 调用；付费行为只在显式 smoke/qualification suite 执行，不能把测试 Session 写入用户历史。
...
- Pi 的 `managed_system_prompt` 是第三种 Bootstrap delivery mode，不改变既有 Bootstrap 或 Formatter 22 原始 Dynamic Context。v5 extension 在每个 `before_agent_start` 的自身 hook 位置把完整 Bootstrap 追加到 Pi 当前 system prompt；后加载的原生 extension 可能继续改写最终 provider payload，Rovai 不声明完整 prompt attestation。只有 closed V2 receipt 对 Host/run/epoch/binding/session/delivery/prompt/cwd、Bootstrap/binding digest 和 `bash/edit/write` 三个 governed Tool 的最小观察全部匹配，Core 才能在同一事务写入不可变 receipt 并接受 Input。Receipt、binding 和 Runtime state 不包含 MCP 字段或 Session locator。direct human slash command 和已授权图片使用独立 Pi transform/image 私有证据在 dispatch 前冻结，不改写原 ContextManifest。Pi system prompt 独立于压缩消息历史，因此固定使用 `native_system_prompt_preserved`，不创建 redelivery requirement 或 compaction observer lease。
...
- 上游 extension/Tool API 的存在不要求 Rovai 自建 transport bridge。Pi managed extension 只保留绑定、Bootstrap、
  managed Skill root 追加、`bash/edit/write` 部分 Approval、最小 receipt 与 Session 状态职责；Pi 自身 extension/Tool

...
- 发送后的附件是 Camp 公共资源，拥有唯一应用受管稳定路径，不位于用户 Workspace，且不继承原文件变化。发现集合、目录树和 Runtime 路径在发送时冻结；消息寻址不缩小 Camp 附件授权。
...
- 静态 discovery/rescan 只允许路径、权限、文件身份和 Adapter 声明的无副作用有界身份命令。纯找到可执行文件是 `found_uninspected`；身份命令成功才可形成非 TRAE 的 `light_ready`，但二者都不声称认证、协议、模型、Session 或 capability Ready。启动、页面打开、成员选择、过期和重扫不自动深检；深检只由用户明确检查或首次真实 Run admission 触发。
...
- TRAE 参加普通 light discovery：启动与显式 rescan 可以经受限 Probe process owner 执行有界 `traecli --version`，成功形成 `light_ready`，失败形成 `light_failed`。用户点击“检查可用性”显式授权一次 `AvailabilityCheck` ACP Host，只执行 version、initialize、`session/new` 与动态目录校验，不发送 Prompt、模型请求、工具或 Approval；首次真实 Run 仍从同一执行 Host 的 initialize/Session 结果建立 capability evidence 后继续投递。Installation refresh、health/diagnostics 和 dispatch preflight 不得启动 TRAE。
...
- 队员 Runtime 权限默认是 Adapter 明确支持且已验证的产品选择。TRAE 与 Kiro 的最高权限默认、trust-all/permission mode 映射和 schema digest 都必须由精确 Adapter 合同冻结，不从字符串或版本猜测。
...
- Shared Conversation 始终属于一个 Camp，动态 continuation 使用有界公共消息而不复制私有历史。Task reference、Session continuity、accepted-input/outcome uncertainty、Gather generation、delegation budget 和其他本 Run 事实进入 closed、typed `RUN_FACTS`，取代自由文本 `RUN_NOTICES`；字段缺失与值 unknown 必须可区分。

...
曾离开的 Agent 再次添加会获得同一 CampMember 行的新 version，但产品语义仍是普通添加。
...
取消请求并进入 reconciliation。提交后所有 Agent 业务工具、Delivery materialization/retry 和 publication 都按
旧 exact membership version fail closed。

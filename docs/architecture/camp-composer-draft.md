...
[Camp Composer Draft v3](../contracts/camp-composer-draft-v3.md)；长期取舍见
...
exact Draft send
  ...
  -> load exact ordered ready Prepared Attachment IDs
     -> non-empty body OR at least one ready attachment
        ...
        -> publish and consume through the existing atomic boundary
...
attachment-only accepted
  ...
  -> keep message_attachment / CampTurn / AgentRun in the same transaction
```

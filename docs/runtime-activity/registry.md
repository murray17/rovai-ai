...
Migration 142 在 migration 141 与 v1.53 来源完整成立后，把新 operation 的 current classifier 原子切换到
`activity-v3`，Data Contract 为 `v1.54 / projection schema 93`。已有 `activity-v1` 或 `activity-v2` row 不回填、不重分类、不删除；同一
operation 已有旧 projection 时，后续 phase 继续用原 classifier 结算。Read Side 依次接受 v3、v2、v1。
marker 与 migration receipt 必须在同一事务提交，启动 reconciler 只有在 receipt 142 存在时才接受 v3。

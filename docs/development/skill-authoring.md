...
3. 修改 pinned third-party Skill 时，继续服从其 provenance、许可以及当前 Architecture/Contract 允许的改动面。
...
6. 运行 `git diff --check`；修改本文或文档导航时再运行 `pnpm docs:test` 和
   `pnpm docs:check`；需要验证历史 Decisions 冻结时以真实 base 运行 `pnpm docs:check:ci`。

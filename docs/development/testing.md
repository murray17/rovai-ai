...
| `pnpm smoke:missing-send-recovery` | 十三种产品 Runtime（含 Pi、Kimi、Grok）；Cursor Disabled | 每种 Runtime 使用独立临时 data-dir/Git workspace，真实执行 zero-send 与 accepted-send suppression；ACP 额外执行 tool→final 并生成独立协议 fixture。Kimi/Grok 与其他 ACP Runtime 共用公开 assistant stream、严格 candidate/抑制规则和完整正文 digest；Pi 的独立 tool→final 严格资格缺口见 v1.30 Checklist 报告；provider `<think>` 不清洗、不重分类 |

...
| `pnpm smoke:builtin-cli` | 默认十三种产品 Runtime；Cursor 未准入 | 首个选中 Runtime 的先导 AgentRun 先通过真实 `rovai` lease 产生一条 Public A2A，并证明对应 Message Delivery 与 publication event；同一历史 Camp 另写真实文件附件。随后另一 Camp 的真实 AgentRun Manifest 冻结该历史 Camp，并以自己的 lease/context 执行 `history.search`、显式历史 `camp.search` 与 `camp.read item`，核对同一 A2A identity 及附件 `kind/fileCount`。每个真实 AgentRun 其余只使用固定业务命令，调用十五项 CLI operation；Gather case 额外验证成员公开回传被 capture、Lead 不逐条唤醒且只创建一次 completion。其余仍验证旧 send 输入拒绝、Projection/schema、冲突 recovery、release fence、Replay 与后续 AgentRun 新 lease；transport-independent indeterminate 由 CLI response-loss test 覆盖。Kimi 与 Grok 已通过完整矩阵；Pi 的合并后严格验收状态见 v1.29 Checklist 报告 |
| `pnpm smoke:skills` | Codex 默认；`all` 为十三种产品 Runtime | `ROVAI_SKILL_SMOKE_ADAPTERS=all` 逐一尝试十三组真实投递、发现与消息局部注意力；Cursor `.cursor/skills` 为 DocumentationOnly。Pi `.pi/skills`、Kimi `.kimi-code/skills` 与 Grok `.grok/skills` 的基础投影、发现和调用 marker 已通过并进入 `all`；完整更新/删除隔离资格按各版本 Checklist 单独记录；`--to-user` 仅为隐藏兼容 alias |
...
| `pnpm smoke:mcp-projection` | Codex、Pi、Claude Code、OpenCode、Copilot、Kiro、Qoder、CodeBuddy、Qwen Code、TRAE、Kimi、Grok | 通过真实 Core、Assignment、AgentRun Projection 与 ContextManifest 验证 Adapter-specific 策略。Pi 通过 Core-owned bridge 调用 assigned stdio Tool、逐次 durable Approval，并断言 HTTP 为 adapter unsupported；Grok 使用私有进程 Plugin保留 `NativeWinsSkip`；Kimi 额外覆盖 stdio、Streamable HTTP 和第二个 stdio Server；默认十二种 |
...
| `pnpm smoke:missing-send-recovery` | 十三种产品 Runtime（含 Pi、Kimi、Grok）；Cursor Disabled | 每种 Runtime 使用独立临时 data-dir/Git workspace，真实执行 zero-send 与 accepted-send suppression；ACP 额外执行 tool→final 并生成独立协议 fixture。Kimi/Grok 与其他 ACP Runtime 共用公开 assistant stream、严格 candidate/抑制规则和完整正文 digest；Pi 的独立 tool→final 严格资格缺口见 v1.29 Checklist 报告；provider `<think>` 不清洗、不重分类 |

...
“显示最近输出 / 打开执行台 / 停止执行”；终态标题改为已完成、执行失败或已停止，并移除停止入口。
...
顶部只保留 Camp 名与“只读”，随后显示当前选中 Run 的触发消息和该队员的连续历史时间线。所有 Run 正文、command、
公开结果与文件变化默认展开；点击 Run 标题只切换顶部触发消息，不折叠内容。页面无控制、分页或解释性图例，桌面与手机
使用同一阅读顺序，无横向滚动；状态有文本、可见键盘焦点和最小 44px 手机点击区域。

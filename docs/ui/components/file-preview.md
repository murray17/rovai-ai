...
- 点击继续使用既有来源校验，打开/激活文件 Tab；同一文件去重，支持 `:44-46` 行范围，定位起始行并高亮范围。
  不支持格式仍调用默认应用；来源已校验的目录在 Finder／资源管理器中显示，不进入文本预览。
  消息中明确指向项目外目录的绝对路径、`~/` 路径或本机 file URI 也直接系统显示，不弹文件读取的目录授权选择器。
...
所有 Tab（含 File Change）统一为 180px，不随文件名或选中状态改变宽度；放不下时继续使用标签栏横向滚动。
长文件名在末端 16px 内渐隐，不使用省略号，也不影响类型图标、更新标记或关闭按钮。短文件名保持清晰，
完整文件名保留在可访问名称中，完整路径仍通过悬停提示和选中后的路径行查看。

...
| Composer 与引用/待发送 · G | 结构化 Mention、回复、引用、附件、revision 校验、待发送编辑与冲突反馈 | 独立草稿、revision、发送、待发送与显式接管已接通；待发送追加上传仍有缺口 | `StructuredMentionComposer`、`CampWorkspace`、`PendingCampInputs`、草稿协调器 | 草稿独立身份及后端归属；现有 draft/pending 服务绑定 client 与 Camp，发送原子消费 | P：缓存与输入焦点属客户端；不跨端同步草稿 | 两标签页不同内容/附件互不覆盖；伪造归属失败；陈旧 revision 不消费新内容；断网按原回执核对 |
...
| 必要设置 · G/P | 外观、个人资料、执行台偏好及桌面集成 | 已复用生产主题与导航偏好模型，按 origin/Owner 存展示偏好；完整设置页待适配 | 生产设置页内业务 sections，主题/profile/偏好适配 | Host 业务设置按需提供；本地偏好留客户端 | P：登录/撤销/连接属 Web；托盘、窗口、更新、Desktop Web 开关留 Desktop | 同一业务设置一致；不同客户端主题/导航偏好不互相覆盖；失效会话和迟到响应可恢复 |

...
这只是 Renderer read-model grouping，不创建 Process 领域对象，也不合并 AgentRun。当时 Inspector
...
- Camp 主区使用 `50px Header + minmax(0, 1fr) 阅读区 + 审批停靠区 + Composer`。
  普通窗口继续使用 `min(790px, 可用宽度 - 54px)` 阅读列和最大 690px 的 Agent 正文；窄窗口
  只减少两侧留白，不改变信息顺序。`min-width: 1800px` 时会话工作列与 Composer 都扩展至
  `min(1040px, 100%)`：自然语言叙述限制在约 `76ch`，安全 GFM 的代码块、表格、Task 和审批等
  现有结构化工件可以使用最多 930–1040px 的工件宽度，不把额外空间转换成超长文本行。
- 会话阅读区与输入停靠区使用白色 `--conversation-surface`。用户和 Agent 的普通正文共用同一
  开放阅读平面；消息自身不再按 `user / agent` 获得持久背景、边框或圆角。Hover、键盘 Focus
  可以显示统一的极弱 Steel 临时反馈，Task、代码、表格、附件与执行证据等结构化内容仍按真实
  内容类型拥有各自表面。
- 主阅读流使用同向左对齐布局。用户消息、Agent 消息和 A2A 消息按持久顺序连续
  排列，以固定头像列、显示名称、Runtime、时间及 A2A metadata 表达身份。相邻且作者相同的真实
  消息只收紧垂直间距；每条消息仍保留自身 metadata，不生成回合、线程或合并消息。
- 当 Agent 作者仍是当前 Camp 内可操作队员且 AgentProfile 可解析时，消息头像与显示名称都是同一
  非模态锚定人物信息卡的触发器；单击、Enter 或 Space 打开，`Esc` 关闭并在键盘路径中返回原触发器。
  该操作不切换 Camp、不打开 Inspector、不导航到队员页，也不显示全局 Toast。已离开、已移除或不可解析作者继续显示冻结
  头像与名称，但不聚焦为按钮、不打开信息卡也不恢复寻址能力。
- 队员身份色只点缀头像和名称，不按角色给整段消息铺不同底色；禁止恢复仅用户消息有底色的
  非对称语法，也不得用不存在的 Agent 默认色生成消息背景。
...
- 每条用户和 Agent 正文支持浏览器原生文本选择；鼠标在正文上按下并拖动可选中
  ...
  操作，入口固定在消息内容列右上角，不得随正文长度、宽屏工件或 A2A footer 移到中部或底部；
  入口仅在整条消息悬停或聚焦时显示。复制使用当前显示名称，不暴露内部
...
- 文件读取、文件写入、命令执行及其错误都是 Tool Call。默认行只显示动作图标、
  ...
  把命令或错误升级为独立时间线卡，也不逐行重复显示“Core 已验证”或“Runtime 报告”来源标签。
...
- 在 Composer 普通文本位置直接键入 `@` 时，无论它前面是空格、中文、英文或标点，
  ...
  `@文字`。候选选中后默认在 Mention 后补一个普通空格，并把光标放到空格之后；
  若替换范围后已经是空白，则不重复插入，并把光标移到首个空白之后。Paste 的纯文本
  `@文字` 不打开菜单，也不创建 Mention。
...
- 复制包含 Mention 的正文时，标准剪贴板 `text/plain` 始终使用当前可见文本，例如
  `@小河狸`，复制到其他应用时不暴露内部身份字段。用户历史消息的整条复制入口会为其中
  所有完整 Mention 附加应用内部结构化身份；浏览器原生选区复制继续只保留系统提供的格式，
  只复制 Mention 的一部分时只保留普通文本。
...
- Approval Dock 是普通 Camp 中唯一审批决定 surface，继续读取同一权威对象和决定命令；不能创建
  第二份本地队列、改变顺序或重复提交。Header 与通知摘要的审批数来自同一队列，并只负责展开、
  定位和聚焦 Dock。
...
- Camp 右侧详情栏只保留两个手动激活的页签：“任务 / 队员”。不得保留“上下文投递 / 审批 /
  活动 / 审计”的隐藏 Tab、空白占位、旧 route 或无使用者 state。
...
- “队员”页按 `memberOrder` 展示当前 Camp 中 active 且 profile-not-removed 的 CampMember，并从
  AgentProfile 读取真实 Runtime readiness。每行只读展示身份、团队角色、在队/暂离和 Runtime；
  不复制队员管理页的身份、Presence 或 Runtime 配置 mutation。
- “队员”页提供唯一 Camp-local Default Lead 选择器，并继续提交既有 versioned
  `camps.changeDefaultLead`。只有 active、`profilePresence = present` 且没有 pending leave request
  的队员可选；暂离、正在暂离、已离开和已移除队员不得成为 Lead。
- ContextManifest、Context Delivery Profile 与 Runtime Input Delivery Evidence 继续由 Core 与
  Snapshot 保留，但不投影到 ordinary Inspector；删除可见页签不代表删除、合并或改写证据。
- Approval 只在 Composer 正上方的唯一 Dock 中决定。Header 与通知摘要可在 Inspector 隐藏时直接
  展开、滚动并聚焦 Dock，但不得改变 Inspector 显隐或当前页签。
...
- Inspector 首次使用默认展开，后续在本机记住显示偏好；该偏好不写入 Core，不产生
  Camp 事件或审计。Header 的 Approval 摘要在隐藏时仍可用，但只定位 Approval Dock；Header
  不提供执行过程入口。
...
- Camp 顶栏右侧只承载待审批摘要和 Inspector 显示/隐藏按钮，不渲染执行入口、“停止”按钮或
  `•••` 操作菜单。
- 没有 pending Approval 时不渲染空徽标。Approval 摘要展开、定位并聚焦 Composer 正上方的
  Approval Dock，但不能在 Header 直接执行停止、审批、置顶、重命名、复制会话 ID 或删除，也
  不能打开或切换 Inspector。
...
- App Shell 顶部明确分为三种结构：Camp 继续使用显性 50px `AppHeader`；
  Quick Chat 与设置叠加独立、纯结构的 50px 隐形拖拽栏，内容跨越 App Shell
  两行；队员与记忆不渲染独立空白拖拽栏，右侧页面直接跨越两行并以 3px Steel
  顶边进入内容，不能在侧栏品牌区旁形成无信息的 50px 空白带。
- 队员的 `member-detail-header` 与记忆的 `memory-library-header` 自身承担 `drag`
  表面；其中按钮、链接、输入、菜单、`summary` 及其他交互元素必须明确为
  `no-drag`。页面提示与错误位于 Header 之后。此处页面级 Header 是唯一右侧顶部，
  不得再在它之前恢复通用 AppHeader 或结构性占位条。

## 队员

### 页面结构

- 队员是统一侧栏一级页面，不隐藏侧栏；右侧内容跨越 App Shell 两行，从窗口顶边的
  3px Steel 结构线直接进入 30px 页面留白与详情 Header，不复用通用 AppHeader，
  也不保留独立 50px 空白拖拽区。未选中或列表为空时仍显示“队员 / 从左侧选择或创建队员”
  的 Header 骨架。
...
- 记忆是统一侧栏一级页面，右侧内容跨越 App Shell 两行，从窗口顶边的 3px Steel
  结构线直接进入 30px 页面留白；不渲染独立 50px 空白拖拽区。Header 使用
  `MEMORY / GOVERNED CONTEXT`、标题“记忆”和现有说明“所有正在沿用的记忆都立即生效；
  形成来源仅用于说明和审计。”，右侧底对齐“导出…”与“＋ 新增记忆”；不再显示
  “可回看 · 可修订 · 可遗忘”副标题。
...
- 设置继续使用 App Shell 的 270px 侧栏槽位，但设置导航完整覆盖普通 App 导航，
  ...
  “返回 App”、设置说明和三组导航：“应用”组为“通用 / 外观 / 通知”，“能力”组为
  “Skill / MCP / Agent 运行时”，“支持”组仅为“诊断与修复”。导航只包含这七项，
  不显示“关于与更新”或其他占位分类。
- 分组标题是不可交互的静态文本，使用弱化文字色；组内继续使用既有设置按钮、图标、
  hover、当前页高亮和键盘行为，组间使用 16px 间距。
...
- 保留现有 Settings App Shell、侧栏结构和侧栏交互；HTML 交互稿只约束 Skill 内容区，
  不替换设置导航。内容区 Hero 标题“Skill 管理”，说明同时覆盖 Rovai 内置、固定上游的
  第三方与用户导入 Skill，并明确来源、Runtime 投递范围和启停状态彼此独立；右侧只显示
  “应用全局配置”。

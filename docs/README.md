...
| 修改 AgentRun 局部停止入口、直接提交、权威停止状态，实际模型观测、首值冻结、默认策略回退、执行台位置偏好、进入恢复或模型展示 | [Runtime Catalog 与 Installation 不变量](architecture/foundational-invariants.md#runtime-catalog-installation)、[产品/执行表面不变量](architecture/foundational-invariants.md#product-execution-surface)、[Runtime Catalog Boundaries](architecture/runtime-catalog-boundaries.md)、[Run Process Detail Surface v16](contracts/run-process-detail-surface-v16.md)、[Camp Open Projection v6](contracts/camp-open-projection-v6.md)及[Camp 会话工作区](ui/components/conversation-workspace.md) |

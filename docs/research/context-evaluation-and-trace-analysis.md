...
已确定方向是 Gate 前／每周真实任务回归，以及每日分析前一自然日 Trace。下文的具体指标口径、
CLI 名称、样本与通过标准属于待确认设计；没有新增命令、定时任务、Schema、模型输入或产品界面。
...
建议先构造以下八个 Case 家族，再依据实际运行成本固定代表样本与重复次数；这不是预先声称已有
八个可运行 Case，也不要求每个家族穷举所有变体：
...
3. 冻结少量专项 Case 和上下文对照 Profile，实际校准执行成本后再确认正式样本量、预算和阈值。

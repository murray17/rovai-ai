...
revision 3 的 implementation baseline 为 `main@72ecbce9f4892433727d01e348da9a56c5926863`；旧实现只读取
...
| Bootstrap / Context | Session Charter/Identity/Memory 位于 system/developer 等价高权限层；每 Prompt 绑定冻结 Manifest、Bootstrap、membership、attachments、Skills、delivery、epoch、binding | managed extension 在 `before_agent_start` 追加 `managed_system_prompt`；旧 V1 receipt 试图证明完整 base/effective prompt、Skills 与 native Tools | 官方 `before_agent_start` 可读取 chained system prompt 与结构化 prompt options，并返回替换后的 system prompt；后加载 extension 仍可继续修改 | confirmed revision 3：v5 在自身 hook 位置追加 exact Bootstrap；closed V2 receipt 在 provider 前只验证最小 binding/session/cwd/Bootstrap 和三个 governed Tool，并与 Input accepted 原子提交。原始 Formatter 22 与实际 Pi transform bytes 都在 dispatch 前私有冻结；不声明后续 extension 未改写 provider payload | 后加载 extension mutation、compaction 后 receipt 与 replacement Session 的完整真实 Golden Flow 尚未完成 |
...
| Skills / Context / Templates | 复用 Skill Library 与 delivery group，同时不覆盖 Runtime 自有资源；资源调用入口必须真实可用 | `pi -> .pi/skills` 曾作为唯一 Skill 来源，完整 catalog 写入 V1 receipt；原生 Context/Templates/Extensions 被启动参数关闭 | Pi 原生发现用户/项目 Skills、Context files、Prompt templates 与 Extensions；`get_commands` 返回 prompt/skill/extension command 来源 | 正式启动恢复 Pi 原生资源；Rovai managed Skill root 只是额外 path，Core 只验证本轮分配项是 catalog 子集。direct human `/prompt` 与 `/skill` 依据 activation catalog 显式展开，source path/content/expanded bytes 私有留证；未知命令按文本，extension command 明确拒绝绕过 receipt | 原生多资源组合、后加载 extension mutation、compaction 后 catalog 连续性及 macOS x64/Windows 仍需完整真实矩阵 |
...
| Prompt / Images / Queue / Cleanup | accepted 使用 native evidence；图片必须绑定当前授权输入；queue/retry/late event fenced；业务 cancelled 先提交；host quiescent 才进 LRU | 只发送纯文本 prompt；prompt response + V1 receipt 作为 accepted；abort+Fleet stop | RPC 原生支持 `prompt.images`、steer/follow-up/clear_queue、modes、messages/entries、stats/name、compact、auto-compaction 与 export | 当前授权 PNG/JPEG/GIF/WebP 按原顺序发送 exact base64 bytes；模型不支持 image、MIME/digest/size 漂移在 prompt 前失败，每项与集合 digest 私有留证。V2 receipt 与 accepted 原子提交；cancel/late fence 和 planned shutdown 保留；新增原生 RPC wire 已封装，任何未来 steer/follow-up ingress 必须拥有独立 delivery/prompt/receipt | steer/follow-up 产品 ingress、真实 queue/retry late event、compaction/export 授权、crash/invalid JSON/timeout、idle eviction、packaged shutdown 与 Core crash recovery尚未完成 |
...
- 正式 Pi 恢复原生 Built-in tools、Extensions、Skills、Context files 与 Prompt templates；v5 extension 只做绑定、
  Bootstrap、最小回执和 `bash/edit/write` 部分审批，不再固定完整 Tool/Skill catalog。

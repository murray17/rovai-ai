...
| Bootstrap / Context | Session Charter/Identity/Memory 位于 system/developer 等价高权限层；每 Prompt 绑定冻结 Manifest、Bootstrap、membership、attachments、Skills、delivery、epoch、binding | managed extension 在 `before_agent_start` 追加 `managed_system_prompt`；旧 V1 receipt 试图证明完整 base/effective prompt、Skills 与 native Tools | 官方 `before_agent_start` 可读取 chained system prompt 与结构化 prompt options，并返回替换后的 system prompt；后加载 extension 仍可继续修改 | v6 在自身 hook 位置追加 exact Bootstrap；closed schema-3 receipt 在 provider 前只验证最小 binding/session/cwd/Bootstrap 和三个 governed Tool，并与 Input accepted 原子提交。Formatter 22 payload 逐字节成为 `prompt.message`，不再有 Pi transform；不声明后续 extension 未改写 provider payload | 后加载 extension mutation、compaction 后 receipt 与 replacement Session 的完整真实 Golden Flow 尚未完成 |
...
| Skills / Context / Templates | 复用 Skill Library 与 delivery group，同时不覆盖 Runtime 自有资源；资源调用入口必须真实可用 | `pi -> .pi/skills` 曾作为唯一 Skill 来源，完整 catalog 写入 V1 receipt；原生 Context/Templates/Extensions 被启动参数关闭 | Pi 原生发现用户/项目 Skills、Context files、Prompt templates 与 Extensions；`get_commands` 可返回 prompt/skill/extension command 来源 | 正式启动恢复 Pi 原生资源；Rovai 只把 frozen Skill 写入 workspace `.pi/skills`，不通过 extension 追加 path、不读取 `get_commands`、不验证完整 catalog。Pi 原生 ResourceLoader/workspace trust 是唯一发现链，当前 discovery 等级为 `DocumentationOnly`。Rovai `/...` 一律作为普通文本，不读取或展开 Skill/Prompt/Extension command | 原生多资源组合、后加载 extension mutation、compaction 后 catalog 连续性及 macOS x64/Windows 仍需完整真实矩阵 |
...
| Permission / Approval / Workspace | 权限声明必须与实际覆盖面一致；Approval 统一到 Action Safety；allow-once 唯一副作用；deny/cancel 无副作用 | Pi `managed` 曾固定完整 native tool 集并阻止未知 mutation | 上游无 sandbox、无 permission popup；Extension `tool_call` 可执行前 block，但后加载 handler 可继续改写；Tool 使用 Pi 进程 OS 权限 | Pi schema 改为 `partial_managed`；仅 `bash/edit/write` 进入 durable Approval，read/grep/find/ls 和未知 Extension Tool 按 Pi 自身语义执行。Shell identity 来自当前 project trust 的实际 Settings/getShellConfig。既有 allow/deny/cancel 证据保留，但产品不再声明 sandbox、全 mutation coverage 或最终 Tool input attestation | later-handler mutation、read-only/attachment/temp/workspace escape 及 Windows/macOS x64 仍无完整真实安全矩阵；这些缺口继续阻止 qualified |
...
| Prompt / Images / Queue / Cleanup | accepted 使用 native evidence；图片必须绑定当前授权输入；queue/retry/late event fenced；业务 cancelled 先提交；host quiescent 才进 LRU | 只发送纯文本 prompt；prompt response + V1 receipt 作为 accepted；abort+Fleet stop | RPC 原生支持 `prompt.images`、steer/follow-up/clear_queue、modes、messages/entries、stats/name、compact、auto-compaction 与 export | Formatter 22 payload 不变地发送；图片只从 ContextManifest refs/授权取得，按原顺序发送 exact base64 bytes，模型能力、MIME/digest/size 漂移在 prompt 前失败，每项 schema-2 evidence 直接绑定 Delivery。schema-3 receipt 与 accepted 原子提交；cancel/late fence 和 planned shutdown 保留 | steer/follow-up 产品 ingress、真实 queue/retry late event、compaction/export 授权、crash/invalid JSON/timeout、idle eviction、packaged shutdown 与 Core crash recovery尚未完成 |
...
- 正式 Pi 恢复原生 Built-in tools、Extensions、Skills、Context files 与 Prompt templates；v6 extension 只做绑定、
  Bootstrap、最小回执和 `bash/edit/write` 部分审批，不再固定或二次发现 Tool/Skill catalog。

...
| External MCP | 复用 Assignment 与 `PreparedMcpProjection`；按 Run 隔离；logical/runtime name 与冲突策略明确；mutation 经 Action Safety；server 随 cancel/cleanup 退出 | 后期实现 `AdditivePerRun / RovaiWins / CoreManaged` 的 stdio proxy；HTTP 未实现；每次 call durable approval | Pi 核心没有内建 MCP，但官方 Extension 可以注册动态 Tool，因此安全 bridge 可实现 | Core-owned stdio + Streamable HTTP，`AdditivePerRun / RovaiWins / CoreManaged`；secret 不进 Pi 全局配置。真实 0.84.4+MiniMax 覆盖三个 projected Tool、same-name precedence、配置 update、disable/re-enable、unassign/restore、delete 与相邻 Session no-leak；Mutation deny 在 Server dispatch 前阻断，cancel 严格结算 cancelled、回收 stdio Server PID 且无延迟文件。deterministic fixture 另证明 bare stdio command 使用当轮 Runtime PATH，单个 missing command 只记录 unavailable，其他 Server 仍可调用；两个互相等待的 Server 可并发激活，activation timeout 显式回收 stdio 父子进程并按 projection index 稳定合并 | 本机该轴已 `Verified + Implemented`；portable command 尚未以真实 `npx`/Playwright 复跑，macOS x64/Windows 仍需重复平台 Golden Flow，HTTP 中断/网络故障注入仍随 Cleanup 轴待验 |
...
- 统一 Fleet 默认保留 Camp/member-scoped identity；Pi 的显式例外使用 canonical workspace + process digest 复用，当前
  独占 lease 的 Camp/member invalidation scope 随领取更新。可动态刷新的 Session、Prompt、model、Bootstrap、Skill、
  MCP 与 Built-in lease 不进入 process compatibility digest。
